...
2026-08-17 — **Creation now fails safely outside terminals and recovers honestly after open errors.** `amux new` checks both stdin and stdout before creation for the current interactive chat/raw modes. After any post-create open/UI failure it now verifies the created UUID through the held client: only a present agent gets the running/recovery diagnostic, an absent agent returns the underlying error unchanged, and a failed check adds uncertainty without asserting liveness. Recovery commands always target the UUID while a requested name remains display-only. Added scriptable exact-name-or-UUID `amux rm <target>` over the existing deletion RPC, with missing and duplicate-name refusals; daemon retry guidance quotes unsafe targets as one shell argument while leaving plain names readable. Focused boundary/recovery/quoting tests, fmt, clippy with only the two accepted tracked-listener warnings, and all 68 amux-cli tests pass; no golden changed.
