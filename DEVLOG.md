...
2026-08-14 — **Codex threads are now always named, because naming is what persists them.** `amux new codex` with no `--name` failed on first contact with a real machine: `thread/resume failed … no rollout found for thread id`. Codex 0.147's `thread/start` creates a thread in the app-server's memory only — it reports the rollout path it *will* use and writes nothing — and `thread/resume`, which the raw TUI runs at bootstrap, refuses a thread with no rollout. The only call that had ever materialized an amux thread was the `thread/name/set` sitting behind `if let Some(name) = desired_name`, so a named agent worked and an unnamed one could not open raw mode at all. Probing the live app-server found `thread/name/set` and `thread/memoryMode/set` are the only two materializing calls in the entire surface, and `thread/archive` fails with the same error — systemic upstream behaviour, not amux misuse. amux names, because a name is inert where a memory mode is a real setting we would be guessing at, and its effect is visible and reversible.

Three changes, only the first of which is the reported bug. Every thread is now created with a name (`thread_name_for`), an unnamed agent getting the bootstrap label `amux-<8 hex>` on its *thread* while the agent itself stays unnamed and keeps the usual display fallback. Materialization completes *before* `attached` is published, closing an independent race in which a raw subscription arriving between the publish and the rename resumed an unpersisted thread even with `--name`. And the failure is no longer swallowed into a `tracing::warn!`: `CodexAttached` carries `ThreadPersistence`, and `ensure_pty` refuses with a distinct, deliberately non-retryable error rather than letting clients spend 15s retrying something that will never come good — that retry predicate was itself a duplicated fact across a crate boundary, and now reads `amux::codex_io::CODEX_RAW_THREAD_NOT_READY`. Verified live: bare `amux new codex` boots the real codex TUI, and the thread reads back as `amux-27330536` for agent `27330536-…`.

Coverage: the harness now takes `Option<&str>` so an unnamed agent is expressible at all, and C.9 `c9_raw_unnamed` drives the product default — it takes no turn, so it costs no quota. Verifying its red/green surfaced a second problem: the C suite drives the prebuilt `target/debug/amux`, which `cargo test -p amux --test codex_capture` does not rebuild, so my first red run passed against the already-fixed binary. The suite header had said "build the CLI first" since P9. Per this run's own lesson that a written instruction is not a control, the harness now refuses to start against a binary older than any workspace `src/` file — scoped to `src/` so editing the suite itself does not demand a rebuild that would change nothing. Verified three ways: fires on a real source change, silent on a test-only edit, and C.9 red without the fix (`the thread was never persisted … (thread name must not be empty)`) and green with it.
