...
2026-08-22 — **Cloud subscription refusals now degrade visibly without affecting local agents.** `/api/connect` distinguishes `403 {"error":"payment_required"}` from authentication and retriable transport/server failures, while every other `4xx` rejection is terminal instead of entering the reconnect loop. Payment-required signals use a distinct protocol code and separate UI model state, producing an `amux.sh/account` subscription banner while the local fleet remains usable; classification, retry policy, token refresh, reducer state, protocol mapping, and the rendered frame have regression coverage.
