...
presentation values, the fleet cache and the event queue; `app-embedded` owns
an embedded node installation and its relay link; `app-ffi` is the C ABI over
both. The Swift app moved from `ios/` to `apps/apple/`. The merge keeps this
branch's native-stream link, mDNS discovery, account tiers, QUIC carrier and
two-phase pairing, and adds main's work beside them.

A host now announces what kind of machine it is. That `platform` fact rides
the link handshake next to the branch's route and account-binding facts,
reaching `HostEntry` for any host that has been adjacent and staying unset for
one that has not, because a machine whose kind is unknown is not one claiming
to be nothing in particular. A discovery advertisement never sets it: being
found says a machine is there, not what it is.

Pairing keeps only its two-phase form. Main still carried the one-shot
`PairPeer`, `PairPinCloudPeer` and `PairQrCloudPeer` calls this branch had
replaced; they are gone from the schema, the client and the daemon. The app's
`pair_link_now`, which exists so a test driver can hold trust before the
screens that grant it are built, now authenticates the link and commits in one
breath instead. The two pairing error types became one: main's `InvalidPin`
replaced an identically-meaning `Refused`, and its `Transport` variant
absorbed two that both meant the attempt never reached a verdict, passing the
host's own sentence through so a client shows it without translating a code.

The embedded and harness relay dials were ported from gRPC channels to the
branch's carriers. Two model fields that arrived after the committed report
recordings were written are now defaulted, like the other later arrivals, so
those recordings still replay.

2026-09-13 — **Merge main's crate split into the connections branch.** Main
had split the monolithic core into `model`, `wire`, `settings`, `client`,
`host-api`, `node`, `agent-runtime` and a public `testnet` harness, and moved
the task runner to `just`. The branch's native-stream link runtime, mDNS
discovery, tiers and QUIC relay work now live where those boundaries put
them: the link and discovery modules are node internals reached by the
harness only through `node::harness`; the protocol version, message limits
and descriptor tests are in `wire`; `HostVia`, `Tier`, `RelayCarrier`, the
host's `signed_in` fact and the QR payload's address list are `model`
values; pairing candidates and pending peers are `client` types; the relay,
ticket and UDP-proxy fixtures are `testnet` modules; and the `connections`
subcommand is in the `amux` CLI crate. The tunnel pool, subscription
reporter and `LinkService.Connect` that main still carried are gone, as on
the branch. Test-support feature gates no longer exist, so every item the
harness needs is unconditional and hidden rather than gated. The spec chapter
list is grouped by comment lines so rustfmt keeps the reading order.
...
tokens, diff review page, managed-agent `attach` tool, and attachment guide
...
branch's renderer-independent diff documents and Claude feed projection. The
SDK placeholder joins Claude and Codex at the singular `ChatFrameParts`
geometry seam, so composition, scrolling, and mouse hit-testing still measure
the frame actually painted. The deleted TUI exploration fold stays deleted;
its typed-kind fixture intent is covered by the moved `amux-ui` projection
specs. Stable rustfmt, all-target workspace clippy, and the complete workspace
suite pass; clippy reports only the two existing test-only listener helpers.
...
describes server behavior during version negotiation.
...
2026-08-23 — **Several amux instances can now share a machine.** A second daemon (a dev build beside the installed release, or one per git worktree) needed three things it did not have: a way to find its config without a flag, since the Claude Code plugin's hook is the literal `amux hooks claude` and the daemon→PTY→hook chain passes only environment; a config key for the data directory, the one per-instance path (`device.key`, `host_id`, `trust.json`, the materialized Claude marketplace) that was still hard-wired to XDG; and a way to stop `amux new claude` from re-registering the user-scoped Claude Code marketplace against its own data dir. `--config` now also reads `AMUX_CONFIG` (explicit flag still wins), `Config.data_dir` defaults to the old location and is threaded to every former `default_data_dir()` call, plugin setup state lives under `Config.state_path` rather than `State::default_path()`, and `claude.manage_plugin: false` skips plugin registration (the registered plugin's hook still reaches this instance, because `amux` resolves on `PATH`). Defaults reproduce the previous behavior exactly. A committed `.wt.toml` uses the three: every checkout (the main one included) gets its own config under `.wt/amux/` via `AMUX_CONFIG`, `target/debug` first on `PATH`, workspace-wide build/test/lint tasks matching CI, and the daemon as a tree-bound resource. Gate: workspace build, `cargo test --workspace --all-targets`, clippy `-D warnings` with testnet, nightly fmt, client-only build/clippy/tests, spec suite 44/44.

2026-08-23 — **The handoff footprint is now the AGENTS.md trigger alone.** `.handoff.toml` is gone: backend and repo-portfolio configuration had already moved to the operator-global tasks config, and the remaining keys (`theory_homes`, `[contracts] owns`) are declared by the machinery's template but consumed by no skill — absent config yields identical behavior (no standing merge authority, default review depth). The AGENTS.md trigger is also softened: large or consequential work ends with a handoff; small self-contained fixes end with a passing test and a clear commit message.

2026-08-22 — **Dependency graph cleanup: the duplicate HTTP/TLS stack is gone, `codex-sdk` is optional, and the workspace is on current majors.** `oauth2 4` pinned `reqwest 0.11`, which dragged a second copy of `hyper`, `rustls`, `h2`, `http`, `tokio-rustls`, `rustls-webpki`, `webpki-roots` and `base64` into every build; `oauth2 5` uses `reqwest 0.12` (which we keep — `reqwest 0.13` would recreate the split until `oauth2` follows) and its device flow is now driven through an explicit redirect-refusing `reqwest::Client`. `tonic 0.12 → 0.14` with `prost 0.14` (codegen moves to `tonic-prost-build`, the generated code needs `tonic-prost`, `tonic::body::BoxBody` became `Body`) finishes the dedupe of `tower`; tokio's `io-std` feature had only ever arrived transitively and is now declared. `codex-sdk` becomes optional under `local-agents` — its sole consumer, `agents/codex/session.rs`, was already gated — so embedded client builds stop compiling the Codex app-server client and its WebSocket stack. `e2e-runner` takes its versions from the workspace (it was pinning `portable-pty 0.8` beside the workspace's 0.9). Also bumped: `thiserror 2`, `jsonwebtoken 11`, `webpki-roots 1`, `gethostname 1`, `socket2 0.6`, `nix 0.31`, `portable-pty 0.9`, `similar 3`, `ratatui 0.30` + `crossterm 0.29`, and `cargo update` for 150-odd compatible releases. Deliberately not bumped, because each would *add* a duplicate: `curve25519-dalek 5` (`jsonwebtoken → ed25519-dalek` pins 4), `sha2 0.11` and `tokio-tungstenite 0.30` (both pull a second RustCrypto hash chain), `signal-hook 0.4` (`crossterm` pins 0.3); `libc 1.0` and `rustls 0.24` are pre-releases. Client-only graph: 308 → 259 crates; remaining duplicates (`getrandom`, `thiserror 1`, `syn`) are transitive to `jsonwebtoken`/`oauth2`. Measured beforehand on a probe linking the iOS bridge's entry points, the duplicates were all dead-stripped — they cost build time and graph hygiene, not app bytes; the app-size lever is the bridge's release profile (12.1 MB default vs 4.9 MB with fat LTO, one codegen unit, `opt-level = "s"`, `panic = "abort"`). Gate: fmt, workspace clippy `-D warnings`, client-only clippy and build, `aarch64-apple-ios` check, `cargo test --workspace` green, spec suite 44/44, `cargo test -p amux --lib` 455/455 with and without default features.
