...
2026-08-17 — **Creation now fails safely outside terminals and recovers honestly after open errors.** `amux new` checks both stdin and stdout before creation for the current interactive chat/raw modes, and a post-create open failure now reports that the agent remains running with exact attach/remove commands (using the UUID for unnamed agents). Added scriptable exact-name-or-UUID `amux rm <target>` over the existing deletion RPC, with missing and duplicate-name refusals. Focused boundary tests, fmt, clippy with only the two accepted tracked-listener warnings, and all 65 amux-cli tests pass; no golden changed.
