...
- **Chat** — the structured conversation view over a Claude PTY session,
  built on the typed `claude_pty_transcript_v1` protocol: Claude Code's
  transcript rows interleaved with amux hook, keymap, and input-result rows;
  input is a seq-guarded semantic intent that the daemon encodes for the
  session's observed Claude version.
