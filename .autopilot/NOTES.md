...
- Task 40 found no Claude launch-code defect. The QA tmux server dropped this
  worktree's `AMUX_CONFIG` and targeted the default daemon, whose deleted dev
  executable made its immutable MCP route invalid. In tmux always pass
  `--config /Users/jlw/.wt/trees/amux/diffs/.wt/amux/config.yaml` explicitly or
  export it inside the pane. With the worktree daemon, Claude Code 2.1.259 opens
  PTY chat idle and creates the SDK agent/chat placeholder; proof frames are in
  `.autopilot/evidence/live/attachments-qa/task40-{pty,sdk}-open.txt`.
- Task 42 rerun passed the live same-host QA. In a 120x40 `attachments-qa` tmux
  pane, pass the worktree `--config` explicitly inside the pane/session setup.
  Ctrl+V with `images/global_architecture.png` produced an `[Image #N]` token; a
  nine-line bracketed paste produced `[Pasted text #N +8 lines]`; both rode one
  draft and the sent feed showed the image row. Claude described the PNG without
  a permission ask. Replay details and all five frames are in
  `.autopilot/evidence/live/attachments-qa/README.md` and
  `.autopilot/qa/attachments-live-rerun.md`; Preview was confirmed frontmost.
