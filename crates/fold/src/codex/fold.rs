...
/// are ours; model owns the same closed tool-name vocabulary used to author
/// the daemon definitions. Any other server stays generic even when it
...
fn mcp_tool_kind(item: &Value) -> WorkKind {
    ...
    if server == model::AGENT_TOOL_SERVER_NAME && model::AGENT_TOOL_NAMES.contains(&tool.as_str()) {
        if tool == "send"
