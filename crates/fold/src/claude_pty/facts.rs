...
use super::{
    ...
    STRUCTURED_PATCH_LINES_RETAINED,
};
...
impl SuggestionDestination {
    pub fn from_wire(destination: &str) -> Self {
        match destination {
...
/// A landed edit, as the tool result states it: which file moved, by how
/// much, and the patch itself.
///
/// Both Claude chats read this from the same provider JSON. The two
/// carriers spell the sidecar key differently — the transcript writes
/// `toolUseResult`, the stream-JSON session writes `tool_use_result` —
/// so each fold hands over the value it found and nothing else differs.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct LandedEdit {
...
/// The landed edit an Edit or Write result carries, or None when the
/// sidecar is some other tool's.
pub fn landed_edit(sidecar: &Value) -> Option<LandedEdit> {
    if !sidecar.is_object() {
        return None;
    }
    let file_path = sidecar.get("filePath").and_then(Value::as_str)?;
    let patch = sidecar.get("structuredPatch").and_then(Value::as_array)?;
    let (mut added, removed) = patch_magnitude(patch);
    // A Write that CREATED the file carries an empty patch; the honest
    // magnitude is the created content's line count.
    if added == 0 && removed == 0 && sidecar.get("type").and_then(Value::as_str) == Some("create") {
        added = sidecar
            .get("content")
...
pub(crate) fn patch_magnitude(patch: &[Value]) -> (u64, u64) {
    let mut added = 0;
    let mut removed = 0;
    for hunk in patch {
...
pub(crate) fn structured_patch_document(patch: &[Value]) -> Document {
...
pub(crate) fn patch_hunk_counts(lines: &[Value]) -> (u32, u32) {
    let mut old = 0u32;
    let mut new = 0u32;
    for line in lines.iter().filter_map(Value::as_str) {
        match line.as_bytes().first() {
...
pub(crate) fn structured_hunk_header(
    old_start: u32,
    old_count: u32,
    new_start: u32,
    new_count: u32,
