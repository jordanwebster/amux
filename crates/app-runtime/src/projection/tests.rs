...
/// What a machine last said is the only account of that conversation there
/// is, so it survives the machine going quiet: a removal that arrives while
/// the host is offline or away is cached inventory, not a deletion, and the
/// card stays with its folded layer, marked not live. The projected feed
/// keeps its rows until the machine itself replaces them. An agent genuinely
/// gone from a machine still answering loses its rows.
#[test]
fn mobile_projection_keeps_the_feed_of_an_agent_whose_host_has_gone_away() {
    ...
    assert!(
        model.claude(AGENT).is_some(),
        "a removal from an offline host is cached inventory, not a deletion"
    );
