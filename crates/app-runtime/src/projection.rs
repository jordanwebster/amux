        ...
        let fleet = Event::Fleet {
            ...
            agents: model
                ...
                .map(|card| AgentCardDto {
                    ...
                    outcome: None,
                    // A row the connection filled is confirmed; a row the
                    // model kept when its machine went quiet is a memory the
                    // same way a cached row is, and must read as one rather
                    // than pass for something still answering.
                    awaiting: !card.live,
                })
                .collect(),
            hosts: model
