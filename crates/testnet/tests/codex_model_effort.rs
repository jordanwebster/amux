...
use backend_harness::{CodexBackendHarness, CodexSdkV1Input};
use codex::{Codex, CodexConfig, ThreadConfig};
use model::{ApprovalPolicy, SandboxPolicy};
...
async fn model_effort_backend_delivers_recorded_settings_on_prompt_and_empty_turn() {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/codex");
    let (reader, writer, controller) = replay_transport_with_controller(
        load_script(root.join("model_effort.io.jsonl")),
    ...
    if std::env::var_os("UPDATE_MODEL_EFFORT").is_some() {
        std::fs::write(root.join("model_effort.rows.jsonl"), &actual).unwrap();
    }
    assert_eq!(
        actual,
        std::fs::read_to_string(root.join("model_effort.rows.jsonl")).unwrap()
    );
    println!(
