...
use client::Client;
use node::{SendInputRequest, SubscribeSessionEvent, SubscribeSessionRequest};
use testnet::{TestNet, Via};
use uuid::Uuid;

const DEADLINE: Duration = Duration::from_secs(5);

async fn send(client: &Client, agent: Uuid, text: &str) -> Result<(), client::ClientError> {
    client
...
async fn revocation_device_identity_and_peer_fingerprints_survive_restart() {
    ...
    let fingerprint = model::public_key_fingerprint(&phone.identity_on_disk().1);
