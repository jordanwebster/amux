...
use backend_harness::ClaudeSdkBackendHarness;
...
async fn journey(with_catalogue: bool) {
    ...
    let provider = tokio::spawn(async move {
        ...
        for line in include_str!("../../ui-state/tests/fixtures/todos/sdk-rows.jsonl")
            .lines()
            .take(2)
        {
    ...
    assert_eq!(
        ui_state::provider::facts(&model, AGENT).commands[0].name,
        "compact"
    );
    let initialized = ui_state::provider::facts(&model, AGENT);
    assert!(
    ...
    let todos = ui_state::provider::facts(&model, AGENT).todos.unwrap();
    assert_eq!(
    ...
    let facts = ui_state::provider::facts(&model, AGENT);
    assert_eq!(facts.model.as_deref(), Some("sonnet"));
    assert_eq!(facts.effort.as_deref(), Some("high"));
    assert_eq!(
        facts.permission,
        ui_state::provider::PermissionFacts::Claude {
            mode: Some("plan".into())
        }
    );
    ...
    .enumerate()
    {
        ...
        host.wait_for_type("amux.claude_sdk.input_result")
            ...
            .unwrap();
        pump(&mut model, &host, &mut consumed);
        let current = ui_state::provider::facts(&model, AGENT);
        ...
        let replayed = ui_state::provider::facts(&reopened, AGENT);
    ...
    pump(&mut model, &host, &mut consumed);
    assert_eq!(
        ui_state::claude_sdk::send_gate(&model, AGENT),
        ui_state::claude_sdk::SendGate::Ready
    );
