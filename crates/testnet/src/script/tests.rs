use model::{PermissionAnswer, PlanAnswer, QuestionAnswer, QuestionResponse};
...
async fn queued_prompts_are_observed_before_end_turn_and_play_once_in_order() {
    ...
    let pins = vec![
        model::id_of(b"second attachment"),
        model::id_of(b"first attachment"),
    ];
    ...
    assert_eq!(provider.observe()[1].pins, expected_pins);
    assert!(provider.observe()[0].pins.is_empty());
    assert!(provider.observe()[2].pins.is_empty());
    assert_eq!(
        provider
            .observe()
            .iter()
...
async fn asks_have_real_semantic_ids_and_answers_select_the_matching_reaction() {
    ...
    for (ask, kind, answer) in asks {
        ...
        assert_eq!(provider.observe()[1].ask_id.as_deref(), Some(id.as_str()));
        assert_eq!(
            provider.observe()[1].answer,
        ...
        println!(
            "{}",
            json!({"ask":{"id":id,"kind":facts[0].kind},"observed":provider.observe()})
        );
    }
}
...
async fn testnet_agents_shutdown_releases_the_backend_provider_while_executor_lives() {
    let net = crate::TestNet::builder().daemon("host").start().await;
    let (_, provider) = net
        .daemon("host")
