...
use model::{AskAnswer, ClaudePtyIntent as Intent, PermissionAnswer, PlanAnswer};
...
impl Provider {
    ...
    pub fn feed(&self, input: Intent, pins: Vec<model::ArtifactId>) -> Result<(), ScriptError> {
    ...
    /// Every semantic input the daemon accepted for this agent, in order.
    pub fn observe(&self) -> Vec<ObservedInput> {
        self.0.engine.lock().unwrap().observed.clone()
    }

    /// Plays raw transcript rows at the session.
    pub async fn emit(&self, rows: Vec<Value>) -> Result<(), ScriptError> {
        self.play(vec![Step::Rows { jsonl: rows }]).await
    }

    /// Raises one ask and waits for it to reach the session stream.
    pub async fn raise_ask(&self, ask: ScriptAsk) -> Result<(), ScriptError> {
        self.play(vec![Step::Ask(ask)]).await
    }

    /// Ends the current turn.
    pub async fn end_turn(&self) -> Result<(), ScriptError> {
        self.play(vec![Step::EndTurn]).await
    }

    /// Exits the provider with `code`.
    pub async fn exit(&self, code: i32) -> Result<(), ScriptError> {
        self.play(vec![Step::Exit { code }]).await
    }
