...
async fn exercise() {
    ...
    let journey = async {
        ...
        .await;
        let request = node::CreateAgentRequest {
        ...
        for (agent, text) in [(created.id, "created prompt"), (sdk, "existing prompt")] {
            ...
            wait_for(&mut runtime, "SDK observed model", |model| {
                ui_state::provider::facts(model, agent).model.as_deref() == Some("haiku")
            })
            .await;
            let seen = control
        ...
        wait_for(&mut runtime, "PTY ready", |model| {
            ...
            model.is_synchronized()
                ...
                && ui_state::claude::send_gate(model, pty) == ui_state::claude::SendGate::Ready
        })
        .await;
        ...
        assert_eq!(
            ui_state::provider::settings_gate(runtime.model(), pty),
