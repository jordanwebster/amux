...
async fn testnet_codex_offers_models_efforts_and_commands_to_a_connected_client() {
    ...
    let exercise = async {
        ...
        .await;
        let facts = ui_state::provider::facts(runtime.model(), agent);
        assert_eq!(
            facts
                .models
                .iter()
        ...
        assert!(
            facts
                .commands
                .iter()
                .all(|command| command.source == ui_state::provider::CommandSource::Codex)
        );
        assert_eq!(
            ui_state::provider::settings_gate(runtime.model(), agent).refusal(),
            None,
        ...
        wait_for(&mut runtime, "the effort the host now reports", |model| {
            ui_state::provider::facts(model, agent).effort.as_deref() == Some("high")
        })
        .await;
