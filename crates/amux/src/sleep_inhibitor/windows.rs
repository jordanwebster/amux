...
impl SleepInhibitor {
    ...
    pub(crate) fn acquire(&mut self) {
        ...
        match PowerRequest::new_system_required(ASSERTION_REASON) {
            Ok(request) => {
                info!("idle sleep prevention active");
                self.request = Some(request);
            }
            Err(error) => {
                warn!(reason = %error, "failed to acquire windows idle sleep inhibitor");
            }
        }
    }
}
...
impl Drop for PowerRequest {
    fn drop(&mut self) {
        if unsafe { PowerClearRequest(self.handle, self.request_type) } == 0 {
            let error = std::io::Error::last_os_error();
            warn!(reason = %error, "failed to clear windows idle sleep inhibitor");
        }
        if unsafe { CloseHandle(self.handle) } == 0 {
            let error = std::io::Error::last_os_error();
            warn!(reason = %error, "failed to close windows idle sleep inhibitor handle");
        }
    }
}
