...
async fn run_inner(
    mut config: Config,
    initial_chat: Option<node::AgentId>,
    ...
    let tui_config = TuiConfig {
        ...
        default_open_mode: match config.ui.default_open_mode {
            node::OpenMode::Raw => tui::OpenMode::RawAttach,
            node::OpenMode::Chat => tui::OpenMode::Chat,
        },
...
fn profile_diagnostics(config: &Config) -> Option<tui::DiagnosticsSource> {
    ...
    amux::diagnostics::source(config, GIT_SHA, cfg!(debug_assertions), move || {
