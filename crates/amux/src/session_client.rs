    ...
    create_and_open_agent(
        ...
        move |agent_id| async move {
            match (config.ui.default_open_mode, terminal_exposed) {
                ...
                (node::OpenMode::Chat, true) => {
                    crate::ui::run_for_agent(config.clone(), agent_id, codex_configuration).await
                }
                (node::OpenMode::Raw, true) => {
...
fn attach_opens_chat(kind: &node::AgentKind, local: bool) -> bool {
    if kind.exposes(node::Protocol::TerminalV1) && ui_state::AgentLayer::from_kind(kind).is_none() {
        return false;
    }
    !local || matches!(kind, node::AgentKind::Codex) || !kind.exposes(node::Protocol::TerminalV1)
}
...
async fn remove_agent_with_client(
    ...
    force: bool,
    rpc: &Client,
) -> Result<node::DeleteAgentSummary> {
    let agents = rpc.list_agents().await?;
...
fn family_descendants<'a>(agents: &'a [node::Agent], root: &node::Agent) -> Vec<&'a node::Agent> {
    let mut children: HashMap<AgentKey, Vec<&node::Agent>> = HashMap::new();
    for agent in agents {
...
fn removal_child_label(agent: &node::Agent) -> String {
...
fn resolve_remove_agent<'a>(agents: &'a [node::Agent], target: &str) -> Result<&'a node::Agent> {
...
fn agent_key(agent: &node::Agent) -> AgentKey {
    (agent.id, agent.host_id)
}

fn display_name(agent: &node::Agent) -> String {
    agent.name.clone().unwrap_or_else(|| agent.id.to_string())
}

fn sort_agent_indexes(indexes: &mut [usize], agents: &[node::Agent]) {
...
fn agent_list_lines(agents: &[node::Agent], all: bool, now: DateTime<Utc>) -> Vec<String> {
...
pub(crate) async fn attach_loop<W: Write>(
    rpc: Client,
    mut session: node::SessionStream,
    agent: AgentIdentifier,
...
mod attach {
    ...
    fn entry_policy_a_session_without_a_terminal_is_created_and_attached_as_chat() {
        ...
        assert!(!super::attach_opens_chat(&node::AgentKind::TestAgent, true));
    }
    ...
    fn listed_agent(id: u128, name: &str) -> node::Agent {
        node::Agent {
    ...
    fn child_agent(id: u128, name: &str, parent: u128) -> node::Agent {
        let mut agent = listed_agent(id, name);
        agent.parent = Some(node::AgentParent {
    ...
    fn a2a_list_collapses_families_and_states_current_work() {
        ...
        parent.working_on = Some(node::WorkingOn {
    ...
    fn a2a_rm_cascade_refuses_working_children_without_force() {
        ...
        working.working_on = Some(node::WorkingOn {
