...
trait DaemonApi: Send + Sync {
    ...
    async fn list_hosts(&self) -> Result<Vec<node::HostEntry>>;
...
mod attach_tests {
    ...
    struct FakeDaemon {
        ...
        created_type: Mutex<Option<node::AgentType>>,
    ...
    fn test_agent(id: Uuid, host_id: Uuid, name: &str) -> Agent {
        agent_with_kind(id, host_id, name, node::AgentKind::TestAgent)
    }
    ...
    async fn a2a_mcp_spawn_gives_a_claude_child_the_configured_driver() {
        for driver in [model::ClaudeDriver::Sdk, model::ClaudeDriver::Pty] {
            ...
            assert_eq!(
                ...
                Some(node::AgentType::Claude { driver }),
