...
mod saved_report_replay {
    ...
    async fn saved_report_replay_leaves_a_stopped_installation_stopped() {
        ...
        assert!(!fixture.installation.front_door_socket.exists());
        let profile = node::load_profile_config(&fixture.profile).unwrap().profile;
...
fn config_split_cli_rejects_path_disagreement_before_starting() {
    let fixture = Fixture::new();
    let mut profile = node::load_profile_config(&fixture.profile).unwrap().profile;
...
fn config_split_worktree_generator_uses_stable_uuid_and_profile_alias() {
    ...
    let config = node::load_profile_config(&root.join("profile.yaml")).unwrap();
    assert_eq!(
        config.profile_id.0,
...
async fn front_door_cli_ssh_pair_receiver_uses_its_explicit_profile() {
    ...
    let local_data = node::load_profile_config(&local.profile)
        .unwrap()
        .profile
        .data_dir;
    let mut receiver =
    ...
    let peer = tokio::time::timeout(
        ...
        node::pair_via_ssh_initiator(io, local_data, "local", "remote.example", &local_admin),
    )
    .await
    .unwrap()
    .unwrap();
