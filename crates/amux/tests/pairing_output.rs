...
use node::{Installation, InstallationConfig, OperationId};
...
async fn pairing_cli_prints_pin_expiry() {
    ...
    let installation = Installation::from_config(
        config.clone(),
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if FrontDoorClient::connect_socket(&config.front_door_socket)
                .await
                .is_ok()
            {
                break;
