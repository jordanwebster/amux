...
pub use tui_fixtures::{NamedState, UnknownNamedState, all_states};
pub struct Fixture {
    pub model: Model,
    pub view: ViewState,
    ...
    match state {
        ...
        NamedState::ClaudeSdkExplorationExpanded => {
            ...
            let run = fixture
                ...
                .find_map(|item| match item {
                    ui_state::claude_sdk::FeedItem::ExplorationRun { id, .. } => {
        ...
        NamedState::ExplorationExpanded => {
            ...
            let run = fixture
                ...
                .into_iter()
                .find_map(|item| match item {
                    ui_state::claude::FeedItem::ExplorationRun { id, .. } => {
                        Some(crate::chat::blocks::RunKey(id))
                    }
...
fn review_working_tree() -> ui_state::DiffBase {
    ui_state::DiffBase::WorkingTree
}
...
fn review_fixture(base: ui_state::DiffBase, script: &[ReviewKey]) -> Fixture {
    ...
    pub fn label(&self) -> String {
        match self {
            ...
            Self::FrozenDiff(base) => match base {
                ui_state::DiffBase::WorkingTree => "diff ready (working tree)".to_string(),
                ui_state::DiffBase::Branch { base } => format!("diff ready (branch {base})"),
            },
...
pub fn deliver_diff_response(
    ...
    response: ui_state::DiffResponse,
) {
    let op = ui_state::OpId(uuid::Uuid::from_u128(0x5eed_0000_0000_0004));
    fixture
        .view
        .chat
        .as_mut()
    ...
    update(
        ...
        Msg::OpResult {
            op,
            outcome: ui_state::OpOutcome::DiffReady { response },
        },
    );
    fixture
        .view
...
fn settle_context_request(fixture: &mut Fixture) {
    ...
    let command = Command::ClaudeSdk(ui_state::ClaudeSdkCommand::RequestContextBreakdown {
...
fn fixture_image() -> ui_state::DraftAttachment {
...
fn fixture_file() -> ui_state::DraftAttachment {
    ui_state::DraftAttachment::from_bytes(
...
fn fixture_report() -> ui_state::DraftAttachment {
    ui_state::DraftAttachment::from_bytes(
        ui_state::ArtifactKind::File,
        "coverage.html",
        "text/html",
...
fn attachment_element(attachment: &ui_state::DraftAttachment) -> String {
    let kind = match attachment.kind {
        ui_state::ArtifactKind::Image => ui_state::MentionKind::Image {
            id: attachment.id.clone(),
        },
        _ => ui_state::MentionKind::File {
            id: attachment.id.clone(),
        },
    };
    ui_state::format_mention(&ui_state::Mention {
        kind,
...
/// a name and a size.
fn attachment_refs_row(attachments: &[&ui_state::DraftAttachment]) -> Value {
    json!({
        ...
        "refs": attachments
            ...
            .map(|attachment| json!({
                ...
                "kind": match attachment.kind {
                    ui_state::ArtifactKind::Image => "image",
                    ui_state::ArtifactKind::Diff => "diff",
                    _ => "file",
                },
...
fn claude_attachment_rows() -> Vec<Value> {
    ...
    let pasted = ui_state::format_mention(&ui_state::Mention {
        kind: ui_state::MentionKind::Text {
            lines: 240,
...
fn agent(protocol: StructuredProtocol, name: &str) -> Agent {
    let (command, kind) = match protocol {
        ...
        StructuredProtocol::Codex => ("codex", ui_state::AgentKind::Codex),
    };
    Agent {
...
fn sdk_family_fixture() -> Fixture {
    ...
    let child_of_lead = |protocol: StructuredProtocol, name: &str| Agent {
        parent: Some(ui_state::AgentParent {
