...
use ui_state::BaseIdentity;
use ui_state::review::{Review, ReviewComment, ReviewHeader, Side, parse_base, parse_stored_patch};
    ...
    for (index, file) in core.document().files.iter().enumerate() {
        ...
        for (row, group) in groups.into_iter().enumerate() {
            ...
            lines.extend(super::comments::thread_lines(
                core.comments(),
                &rows,
                ui_state::review::RowRef { file: index, row },
                width,
                theme,
            ));
        }
