...
use ui_state::claude_sdk::{AskState as SdkAskState, ClaudeSdkCommand, SdkPhase};
use ui_state::{AgentId, ClaudeCommand, CodexCommand, Command, Model, StructuredProtocol};
...
pub(crate) fn handle_key(model: &Model, inline: &mut InlineAsk, key: &KeyEvent) -> InlineOutcome {
    ...
    match &mut inline.ui {
        ...
        Ui::Codex { cursor } => {
            ...
            let allows_answer = ui_state::codex::allows_answer(model, child);
            match key.code {
...
fn interrupt(model: &Model, inline: &InlineAsk) -> InlineOutcome {
    ...
    match &inline.ui {
        ...
        Ui::ClaudeSession(_)
            if matches!(
                ui_state::claude_sdk::phase(model, child),
        ...
        Ui::Codex { .. } if ui_state::codex::allows_interrupt(model, child) => {
