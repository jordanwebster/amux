...
//! session layer in `ui_state::claude_sdk`.
...
pub(crate) use render::{
    claude_sdk_frame_parts, stored_entry_block, stored_entry_owner, stored_entry_paints,
};
...
use ui_state::{AgentId, Command, Model, OpId, OpOutcome};
...
fn answer_summary(answer: &SdkAnswer) -> AnswerSummary {
    use ui_state::claude_sdk::{DialogAnswer, ElicitationAnswer};
    match answer {
...
pub(crate) fn answer_command(agent: AgentId, ask: u64, answer: PanelAnswer) -> Command {
    let answer = match answer {
        ...
        PanelAnswer::Claude(answer) => match answer {
            ui_state::claude::answer::AskAnswer::Permission(answer) => {
                SdkAnswer::Permission(answer)
            }
            ui_state::claude::answer::AskAnswer::Plan(answer) => SdkAnswer::Plan(answer),
            ui_state::claude::answer::AskAnswer::Question(response) => {
...
pub(crate) fn allows_answer(model: &Model, chat: &View) -> bool {
    !chat.read_only(model)
        && ui_state::claude_sdk::send_gate(model, chat.agent) == SendGate::NeedsYou
}
...
impl View {
    ...
    pub fn note_dispatched(&mut self, op: OpId, command: &Command) {
        ...
        match command {
            Command::ClaudeSdk(ui_state::claude_sdk::ClaudeSdkCommand::SendPrompt {
                agent,
                text,
            }) if *agent == self.agent => {
            ...
            Command::ClaudeSdk(ui_state::claude_sdk::ClaudeSdkCommand::AnswerAsk {
                agent,
                ask,
                ..
            }) if *agent == self.agent => {
            ...
            Command::ClaudeSdk(
                ui_state::claude_sdk::ClaudeSdkCommand::RequestContextBreakdown { agent },
            ) if *agent == self.agent => {
    ...
    fn reconcile_diff(&mut self, model: &Model) {
        ...
        let document = match ui_state::review::parse_patch(
            &response.patch,
        ...
        let core = ui_state::review::Review::new(document, response.artifact.id.clone());
    ...
    pub fn needs_tick(&self, model: &Model) -> bool {
        matches!(
            ui_state::claude_sdk::phase(model, self.agent),
            SdkPhase::Working
        ) || model
            .claude_sdk(self.agent)
...
pub(crate) fn ask_detail(model: &Model, agent: AgentId) -> Option<String> {
    ...
    Some(match &ask.kind {
        ui_state::claude_sdk::AskKind::Permission {
            invocation:
                ToolInvocation::Bash {
        ...
        ui_state::claude_sdk::AskKind::Permission { tool_name, .. } => tool_name.clone(),
        ui_state::claude_sdk::AskKind::Plan { .. } => "a plan to review".to_string(),
        ui_state::claude_sdk::AskKind::Question { questions } => questions
            .first()
            .and_then(|question| question.question.as_deref().or(question.header.as_deref()))
            .map(head_line)
            .unwrap_or_else(|| "a question".to_string()),
        ui_state::claude_sdk::AskKind::Elicitation {
            server, message, ..
        } => match server {
            Some(server) => format!("{server}: {}", head_line(message)),
            None => head_line(message),
        },
        ui_state::claude_sdk::AskKind::Dialog { dialog_kind, .. } => dialog_kind.clone(),
    })
}
