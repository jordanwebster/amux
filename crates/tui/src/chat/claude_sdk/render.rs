...
pub(crate) fn claude_sdk_frame_parts(
    ...
    let phase = ui_state::claude_sdk::phase(model, chat.agent);
...
fn activity_row(
    ...
    let todo_progress = layer
        .todos()
        .map(|todo| (todo.done, todo.total, todo.current.as_deref()))
        .or_else(|| {
            model
                .chat(chat.agent)
                .and_then(|chat| chat.head.as_ref())
                .and_then(|head| head.summary().todo.as_ref())
                .map(|todo| (todo.done, todo.total, todo.current.as_deref()))
        });
    if let Some((done, total, current)) = todo_progress {
        let mut progress = format!("todo {done}/{total}");
        if let Some(current) = current {
            progress.push_str(&format!(" · {current}"));
        }
        facts.push(progress);
    }
    let running = layer
...
fn footer_line(
    ...
    } else if let Some(refusal) = ui_state::claude_sdk::send_gate(model, chat.agent).refusal() {
...
/// whose they were.
fn subagent_block(
    layer: &ui_state::claude_sdk::ClaudeSdkLayer,
    entry: &FeedEntry,
    cache: &mut PaintCache,
    theme: Theme,
    width: usize,
...
fn entry_content(layer: &ui_state::claude_sdk::ClaudeSdkLayer, entry: &FeedEntry) -> Vec<Segment> {
    match &entry.kind {
...
pub(crate) fn stored_entry_block(
    ...
    let kind = match entry.entry_kind().unwrap_or_default() {
        ClaudeSdkEntryKind::Prompt => {
            ...
            FeedEntryKind::Prompt(ui_state::claude_sdk::PromptEntry {
                uuid,
                text,
                image_count,
                synthetic,
        ...
        ClaudeSdkEntryKind::Message => FeedEntryKind::Message(ui_state::claude_sdk::MessageEntry {
            text,
            finality: stored_finality(entry.finality()),
        }),
        ClaudeSdkEntryKind::Thinking => {
            FeedEntryKind::Thinking(ui_state::claude_sdk::ThinkingEntry {
                text,
        ...
        ClaudeSdkEntryKind::Tool => {
            ...
            let result = entry.tool_outcome().and_then(|bytes| {
                ...
                Some(ui_state::claude_sdk::ToolResult {
                    text: value
                        .get("text")
                    ...
                    edit: details
                        .as_ref()
                        .and_then(ui_state::claude::facts::landed_edit),
                    details,
                })
            });
            FeedEntryKind::Tool(ui_state::claude_sdk::ToolEntry {
                tool_use_id,
                name: name.clone(),
                invocation: ui_state::claude::facts::invocation(&name, &input),
        ...
        ClaudeSdkEntryKind::Task => {
            ...
            let usage = entry.task_usage().and_then(|bytes| {
                serde_json::from_slice::<ui_state::claude_sdk::TaskUsage>(&bytes.0).ok()
            });
            FeedEntryKind::Task(ui_state::claude_sdk::TaskEntry {
                task_id,
        ...
        ClaudeSdkEntryKind::Turn => {
            ...
            let raw = entry
                ...
                .and_then(|bytes| serde_json::from_slice::<serde_json::Value>(&bytes.0).ok())
                .unwrap_or(serde_json::Value::Null);
            FeedEntryKind::Turn(ui_state::claude_sdk::TurnEntry {
                uuid,
                outcome,
                is_error,
                stop_reason,
        ...
        ClaudeSdkEntryKind::Compaction => {
            ...
            FeedEntryKind::Compaction(ui_state::claude_sdk::CompactionEntry {
                trigger,
                pre_tokens,
                post_tokens,
            })
        ...
        ClaudeSdkEntryKind::AgentMessage => {
            ...
            FeedEntryKind::AgentMessage(ui_state::claude_sdk::AgentMessageEntry {
                id,
                context,
                from,
                kind: message_kind,
        ...
        ClaudeSdkEntryKind::Status => {
            ...
            FeedEntryKind::Status(ui_state::claude_sdk::StatusEntry {
                status,
                details: None,
            })
        }
        ClaudeSdkEntryKind::Boundary => {
            let boundary = match body {
                ClaudeSdkBody::Boundary {
                    boundary,
                    session_id,
                } if boundary == "gap" => ui_state::claude_sdk::BoundaryEntry::Gap {
                ...
                ClaudeSdkBody::Boundary {
                    ...
                    ui_state::claude_sdk::BoundaryEntry::ConversationReset {
                ...
                ClaudeSdkBody::Boundary {
                    boundary,
                    session_id,
                } => ui_state::claude_sdk::BoundaryEntry::Ready {
                    session_id,
                    resumed: boundary == "resumed",
                },
                _ => ui_state::claude_sdk::BoundaryEntry::Ready {
                    session_id: None,
                    resumed: false,
                },
            };
        ...
        ClaudeSdkEntryKind::Unrecognized | ClaudeSdkEntryKind::ApiError => {
            ...
            FeedEntryKind::Unrecognized(ui_state::claude_sdk::UnrecognizedEntry {
                row_type,
                detail,
            })
        }
    ...
    let presentation = FeedEntry::restored(key.0, kind, parent_tool_use_id, false);
    paints(&presentation).then(|| {
        entry_block(
            &presentation,
            &[Segment::Prose(entry.text().unwrap_or_default().to_string())],
...
/// alone.
fn turn_error_text(turn: &ui_state::claude_sdk::TurnEntry) -> Option<String> {
