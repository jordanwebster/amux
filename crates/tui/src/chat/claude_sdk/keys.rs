...
use ui_state::claude_sdk::{AskState, ClaudeSdkCommand, SdkPhase, SendGate};
use ui_state::{AgentId, Command, DiffBase, Model};
...
fn interrupt(chat: &View, model: &Model) -> Option<UiAction> {
    matches!(
        ui_state::claude_sdk::phase(model, chat.agent),
...
fn send(chat: &mut View, model: &Model) -> Option<UiAction> {
    ...
    if ui_state::claude_sdk::send_gate(model, chat.agent)
        .refusal()
        .is_some()
    {
        return None;
