...
use ui_state::{AgentId, Command, Model, OpId, OpOutcome, QueueCommand, QueueDelivery};
...
pub(super) fn key(
    ...
    review: Option<&ui_state::review::Review>,
) -> Option<UiAction> {
    ...
    if queued.is_none() && !ui_state::queue::can_hold(model, agent) {
        return None;
    }
    ...
    fn queue_tui_tab_roundtrip_and_rendered_strip() {
        for state in [
            ...
            for (n, words) in [(1, "check the queue"), (2, "run the tests next")] {
                ...
                assert!(
                    ui_state::update(&mut fixture.model, ui_state::Msg::Command { op, command })
                        .is_empty()
                );
