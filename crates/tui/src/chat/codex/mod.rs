//! Codex-native chat view state. It consumes only `ui_state::codex` facts;
...
use ui_state::codex::{AskContext, CodexCommand, CodexPhase};
...
impl View {
    ...
    pub(crate) fn needs_tick(&self, model: &Model) -> bool {
        matches!(
            ui_state::codex::phase(model, self.agent),
