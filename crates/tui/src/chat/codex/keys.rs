...
use ui_state::codex::{CodexCommand, CodexPhase};
...
pub(crate) fn handle_chat_key(
    ...
    if key.code == KeyCode::Char('x') && ctrl {
        return ui_state::codex::allows_interrupt(model, chat.agent).then_some({
...
fn send(chat: &mut View, model: &Model) -> Option<UiAction> {
    ...
    } else if ui_state::codex::allows_prompt(model, chat.agent) {
...
fn approval_key(
    ...
    let allows_answer = ui_state::codex::allows_answer(model, chat.agent);
    match key.code {
