...
use ui_state::{AgentId, Attention, Command, Model, OpId, OpOutcome};
...
pub(crate) fn shared_ask(ask: &Ask) -> SharedAsk<'_> {
    SharedAsk {
        ...
        refusal: ui_state::claude::answer::menu_shape_refusal(&ask.kind)
...
pub(crate) fn reader_context<'m>(model: &'m Model, chat: &'m View) -> Option<ReaderContext<'m>> {
    ...
    Some(ReaderContext {
        ...
        can_answer: ui_state::claude::allows_answer(model, chat.agent),
...
impl View {
    ...
    pub fn note_dispatched(&mut self, op: OpId, command: &Command) {
        ...
        match command {
            ...
            Command::Claude(ui_state::ClaudeCommand::AnswerAsk { agent, ask, .. })
                if *agent == self.agent =>
            {
    ...
    pub fn needs_tick(&self, model: &Model) -> bool {
        let phase_is_working = matches!(
            ui_state::claude::phase(model, self.agent),
            ChatPhase::Working
        );
