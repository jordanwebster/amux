...
use ui_state::claude::answer::{self, AskAnswer};
use ui_state::{ClaudeCommand, Command, DiffBase, Model};
...
fn composer_key(
    ...
    match key.code {
        ...
        KeyCode::BackTab => {
            if ui_state::claude::mode_cycle_gate(model, chat.agent).is_none() {
                return Some(UiAction::Dispatch(Command::Claude(
...
fn send(chat: &mut View, model: &Model) -> Option<UiAction> {
    ...
    if ui_state::claude::send_gate(model, chat.agent)
        .refusal()
        .is_some()
    {
        return None;
...
mod tests {
    use chrono::{DateTime, Utc};
    use serde_json::json;
    use ui_state::{Model, Msg, OpId, ServerMsg, StreamEntry, StreamMsg, update};
    use uuid::Uuid;
        ...
        fold(
            ...
            vec![Msg::OpResult {
                ...
                outcome: ui_state::OpOutcome::Error {
                    error: ui_state::OpError::general("input raced the session"),
                },
            }],
        );
    ...
    fn a_failed_send_never_clobbers_text_typed_in_the_meantime() {
        ...
        fold(
            ...
            vec![Msg::OpResult {
                op,
                outcome: ui_state::OpOutcome::Error {
                    error: ui_state::OpError::general("transport lost"),
                },
            }],
        );
        chat.reconcile(&model);
    ...
    fn answer_of(action: Option<UiAction>) -> ui_state::claude::answer::AskAnswer {
        match action {
    ...
    fn digits_select_and_enter_dispatches_the_answer() {
        ...
        assert_eq!(
            answer,
            ui_state::claude::answer::AskAnswer::Permission(PermissionAnswer::AllowScoped {
                suggestion: 0
            })
        );
    }
    ...
    fn deny_stage_preserves_text_and_enter_carries_the_feedback() {
        ...
        assert_eq!(
            answer,
            ui_state::claude::answer::AskAnswer::Permission(PermissionAnswer::Deny {
    ...
    fn a_single_select_question_submits_on_the_confirmed_selection() {
        ...
        assert_eq!(
            answer,
            ui_state::claude::answer::AskAnswer::Question(QuestionResponse {
    ...
    fn request_changes_requires_feedback_and_q_types_there() {
        ...
        assert_eq!(
            answer,
            ui_state::claude::answer::AskAnswer::Plan(PlanAnswer::RequestChanges {
    ...
    fn plan_approve_dispatches_from_the_reader() {
        ...
        assert_eq!(
            answer,
            ui_state::claude::answer::AskAnswer::Plan(PlanAnswer::ApproveManual)
        );
    }
    ...
    fn retained_ask_reader_gates_actions_but_keeps_navigation_and_close() {
        ...
        assert!(ui_state::claude::allows_answer(&model, agent_id()));
    ...
    fn readonly_ask_model() -> Model {
        ...
        if let Msg::Server(ui_state::ServerMsg::AgentUpserted { agent }) = &mut msgs[2] {
            agent.readonly = true;
        } else {
            panic!("fixture shape");
        }
    ...
    fn a_refused_answer_from_the_reader_surfaces_in_the_docked_panel() {
        ...
        assert_eq!(
            ...
            Some(ui_state::NOT_CONNECTED_ERROR)
        );
    ...
    fn a_successful_send_leaves_no_failure_and_no_pending_watch() {
        ...
        fold(
            ...
            vec![Msg::OpResult {
                op,
                outcome: ui_state::OpOutcome::InputSent,
            }],
        );
        chat.reconcile(&model);
        assert_eq!(chat.send_failure(), None);
...
mod attachments {
    ...
    fn a_plain_draft_still_sends_as_a_plain_prompt() {
        ...
        assert!(matches!(
            handle_chat_key(&mut chat, &model, press(KeyCode::Enter), VIEWPORT, t(0)),
            Some(UiAction::Dispatch(Command::Claude(
                ui_state::ClaudeCommand::SendPrompt { .. }
            )))
        ));
    }
    ...
    fn failed_send_restores_the_draft_with_its_token_and_states_the_failure() {
        ...
        let op = ui_state::OpId(uuid::Uuid::from_u128(99));
        chat.note_dispatched(op, &command);
        super::tests::fold(&mut model, vec![ui_state::Msg::Command { op, command }]);
        super::tests::fold(
            &mut model,
            vec![ui_state::Msg::OpResult {
                op,
                outcome: ui_state::OpOutcome::Error {
                    error: ui_state::OpError::AttachmentMissing {
                        id: missing,
    ...
    fn agent_id_local() -> ui_state::AgentId {
...
#[cfg(test)]
mod review {
    use ui_state::{Msg, OpId, OpOutcome};
    use uuid::Uuid;
        ...
        fold(
            ...
            vec![Msg::OpResult {
                ...
                outcome: OpOutcome::Error {
                    error: ui_state::OpError::DiffUnavailable {
    ...
    fn readonly_model(model: &mut Model) -> Model {
        ...
        fold(
            &mut readonly,
            vec![Msg::Server(ui_state::ServerMsg::AgentUpserted { agent })],
        );
        readonly
    }
}
