...
use ui_state::{AgentId, Attention, Command, FleetItem, Model};
...
pub enum VisibleRow<'a> {
    ...
    PendingCreate {
        name: &'a str,
        agent_type: &'a ui_state::AgentType,
        host: Option<ui_state::HostId>,
    },
}
...
impl VisibleRow<'_> {
    ...
    pub fn card(&self) -> Option<&ui_state::AgentCard> {
        match self {
...
/// from the Model.
pub fn next_agent_name(model: &Model, agent_type: &ui_state::AgentType) -> String {
    let taken = all_agent_names(model);
    (1..)
        .map(|n| format!("{}-{n}", ui_state::agent_type_label(agent_type)))
...
pub enum UiAction {
    ...
    SwitchProfile(ui_runtime::ProfileEntry),
}
