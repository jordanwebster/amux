...
fn working_text(card: &ui_state::AgentCard, now: DateTime<Utc>, budget: usize) -> Option<String> {
...
fn active_failure(model: &Model, view: &ViewState) -> Option<(String, String)> {
    ...
    let ui_state::OpOutcome::Error { error } = &failure.outcome else {
        return None;
    };
...
fn command_verb(command: &Command) -> &'static str {
    match command {
        ...
        Command::ClaudeSdk(ui_state::ClaudeSdkCommand::SendPrompt { .. }) => "send",
        Command::ClaudeSdk(ui_state::ClaudeSdkCommand::AnswerAsk { .. }) => "answer",
        ...
        Command::Claude(ui_state::ClaudeCommand::SendPrompt { .. }) => "send",
        Command::Claude(ui_state::ClaudeCommand::AnswerAsk { .. }) => "answer",
        Command::Claude(ui_state::ClaudeCommand::Interrupt { .. }) => "interrupt",
        Command::Claude(ui_state::ClaudeCommand::CyclePermissionMode { .. }) => "mode cycle",
        Command::Codex(ui_state::CodexCommand::Prompt { .. }) => "send",
        Command::Codex(ui_state::CodexCommand::Steer { .. }) => "steer",
        Command::Codex(ui_state::CodexCommand::Answer { .. }) => "answer",
        Command::Codex(ui_state::CodexCommand::Interrupt { .. }) => "interrupt",
    }
}
...
fn status_line(model: &Model, view: &ViewState, width: usize, theme: Theme) -> Line<'static> {
    ...
    let hints = match &view.mode {
        ...
        Mode::Help | Mode::Hosts => "any key to close".to_string(),
...
fn has_families(model: &Model) -> bool {
    model
        .fleet()
        .iter()
        .any(|item| matches!(item, ui_state::FleetItem::Family { .. }))
}
...
fn confirm_delete_lines(
    ...
    let name = model
        .agent(agent)
        .map(ui_state::AgentCard::display_name)
...
fn confirm_delete_row(
    model: &Model,
    ctx: &FrameContext,
    member: &ui_state::FamilyMember<'_>,
    width: usize,
) -> Line<'static> {
    let theme = ctx.theme;
