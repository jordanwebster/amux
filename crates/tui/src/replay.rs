...
use ui_runtime::report::{
    FrameCapture, Mark, ReplayVerdict, ReportError, ReportHeader, read_frame,
};
...
impl Replay {
    pub fn load(report: &Path) -> Result<Self, ReplayError> {
        let header = ui_runtime::report::read_header(report)?;
...
pub(crate) mod tests {
    ...
    use ui_runtime::report::{ReportDraft, ReportKind, ReportParts, ReportWriter};
