...
pub struct TuiConfig {
    ...
    pub default_agent_type: ui_state::AgentType,
...
/// `ui_state::Client`. The terminal is restored before `attach` runs and the
    ...
    E: futures_util::Stream<Item = io::Result<crossterm::event::Event>> + Unpin,
{
    ...
    let writer = ui_runtime::report::ReportWriter::new(
...
mod tests {
    ...
    async fn switcher_shell_capture_uses_selected_diagnostics_and_destination() {
        ...
        let entry = ProfileEntry {
            id: ui_state::ProfileId(uuid::Uuid::new_v4()),
            label: "Work".into(),
            email: None,
            status: "local".into(),
