...
use ui_state::attachments::{ArtifactKind, DraftAttachment, Mention, MentionKind, format_mention};
use ui_state::review::Review;
...
mod tests {
    ...
    fn paste_with_newlines_grows_the_draft_without_sending() {
        ...
        assert!(ui_state::claude::answer::check_prompt(&c.text()).is_ok());
    }
    ...
    fn paste_expands_tabs_and_the_draft_stays_sendable() {
        ...
        assert!(
            ui_state::claude::answer::check_prompt(&c.text()).is_ok(),
...
#[cfg(test)]
mod tokens {
    use ui_state::review::{Review, anchor, parse_patch};
    use ui_state::{BaseIdentity, DiffBase, DiffFile};
...
+three";
    ...
    fn review() -> Review {
        ...
        let row = ui_state::review::RowRef { file: 0, row: 2 };
    ...
    fn export_renders_the_elements_and_lists_attachments_in_text_order() {
        ...
        let mentions = ui_state::split_mentions(&text)
            .into_iter()
            .filter(|segment| matches!(segment, ui_state::Segment::Mention(_)))
            .count();
