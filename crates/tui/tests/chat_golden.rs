...
use tui::view::ViewState;
use tui::{ChatView, ColorMode, FrameContext, Theme, render};
use ui_state::attachments::ArtifactKind;
use ui_state::claude::answer::{AskAnswer, PermissionAnswer};
use ui_state::claude::{DiffDocument, DiffMagnitude};
use ui_state::diff::{Document, Hunk, Numbering};
use ui_state::{
    ...
    Msg::Stream {
        ...
        event: StreamMsg::Batch {
            ...
            entries: rows
                ...
                .enumerate()
                .map(|(offset, payload)| {
                    StreamEntry::observed(first_seq + offset as u64, at(arrived), payload)
                })
                .collect(),
        },
    }
...
fn chat_scrolled_back() {
    ...
    let mut view = chat_view();
    {
        ...
        chat.set_scroll(tui::chat::FeedScroll::Paused {
            top_line: 3,
            entry_watermark: tui::chat::entry_watermark(&model, agent_id()).saturating_sub(3),
        });
    }
...
fn chat_truncated_top() {
    ...
    view.chat
        .as_mut()
        .expect("chat open")
        .set_scroll(tui::chat::FeedScroll::Paused {
            top_line: 0,
            entry_watermark: tui::chat::entry_watermark(&model, agent_id()),
        });
...
fn chat_echo_sending() {
    ...
    msgs.push(Msg::Command {
        op: op(1),
        command: Command::Claude(ui_state::ClaudeCommand::SendPrompt {
            agent: agent_id(),
...
fn chat_send_failure() {
    let mut msgs = idle_msgs();
    let command = Command::Claude(ui_state::ClaudeCommand::SendPrompt {
        agent: agent_id(),
        text: "and please add jitter docs".to_string(),
    });
    msgs.push(Msg::Command {
    ...
    msgs.push(Msg::OpResult {
        op: op(1),
        outcome: ui_state::OpOutcome::Error {
            error: ui_state::OpError::general(
...
fn unverified_menu_reader_is_readable_but_pager_only() {
    let model = fold(unverified_edit_ask_msgs());
    assert!(
        ui_state::claude::allows_answer(&model, agent_id()),
...
fn chat_ask_pending() {
    ...
    msgs.push(Msg::Command {
        op: op(2),
        command: Command::Claude(ui_state::ClaudeCommand::AnswerAsk {
            agent: agent_id(),
            ask: 0,
...
fn diff_body_buffer(theme: Theme) -> ratatui::buffer::Buffer {
    let lines = tui::chat::diff::reader_rows(&numbered_diff(), GOLDEN_VIEWPORT.0 as usize, theme);
...
fn leader(view: &mut ViewState, model: &Model, code: KeyCode) {
    ...
    for key in [
        ...
        tui::chat::handle_chat_key(chat, model, key, (80, 20), at(IDLE_NOW));
    }
}
...
fn review_buffer(view: &tui::ReviewView, theme: Theme) -> ratatui::buffer::Buffer {
    let backend = TestBackend::new(GOLDEN_VIEWPORT.0, GOLDEN_VIEWPORT.1);
    let mut terminal = Terminal::new(backend).expect("terminal");
    terminal
        .draw(|frame| {
...
fn review_open_view() -> tui::ReviewView {
    let mut view = tui::review::fixture::sample_review();
    view.set_viewport(GOLDEN_VIEWPORT.0, GOLDEN_VIEWPORT.1);
    view
}

fn review_press(view: &mut tui::ReviewView, code: char) {
...
fn review_file_list_view() -> tui::ReviewView {
    let mut view = tui::review::fixture::sample_review_with_comments();
    view.set_viewport(GOLDEN_VIEWPORT.0, GOLDEN_VIEWPORT.1);
...
fn review_folded_view() -> tui::ReviewView {
...
fn review_branch_base_view() -> tui::ReviewView {
    let mut view = tui::review::fixture::sample_review_against(ui_state::DiffBase::Branch {
        base: "main".to_string(),
    });
    view.set_viewport(GOLDEN_VIEWPORT.0, GOLDEN_VIEWPORT.1);
    view
...
fn review_selection_view() -> tui::ReviewView {
    tui::review::fixture::sample_review_selecting()
}
...
fn review_comment_box_view() -> tui::ReviewView {
    tui::review::fixture::sample_review_commenting()
}
...
fn review_threads_view() -> tui::ReviewView {
    let mut view = tui::review::fixture::sample_review_with_comments();
    view.set_viewport(GOLDEN_VIEWPORT.0, GOLDEN_VIEWPORT.1);
        ...
        vec![
            ...
            prompt_row(
                ...
                &format!(
                    ...
                    tui::review::fixture::sample_review_element(),
                ),
            ),
            turn_duration_row(2, "2026-08-12T09:01:42Z", 102_000),
        ],
...
fn review_reader_fetched() {
    ...
    let command = Command::FetchDiff {
        agent: agent_id(),
        id: tui::review::fixture::sample_diff_id(),
    };
    ...
    update(
        ...
        Msg::OpResult {
            op: op(4),
            outcome: ui_state::OpOutcome::DiffFetched {
                id: tui::review::fixture::sample_diff_id(),
                patch: tui::review::fixture::sample_patch().to_string(),
            },
        },
    );
...
fn open_review_page(view: &mut ViewState, model: &mut Model) {
    ...
    update(
        ...
        Msg::OpResult {
            op: op(4),
            outcome: ui_state::OpOutcome::DiffReady {
                response: tui::review::fixture::sample_diff_response(DiffBase::WorkingTree),
            },
        },
    );
    view.chat.as_mut().expect("chat open").reconcile(model);
