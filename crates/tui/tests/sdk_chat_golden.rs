...
fn asking_child() -> Model {
    ...
    child.kind = ui_state::AgentKind::Codex;
    child.command = "codex".to_string();
    child.parent = Some(ui_state::AgentParent {
        agent_id: agent_id(),
        host_id: host_id(),
    });
...
fn mid_task() -> Model {
    ...
    for (index, row) in recorded("tasks").into_iter().enumerate() {
        ...
        let running = model
            ...
            .expect("the session layer")
            .tasks()
            .any(|task| matches!(task.state, ui_state::claude_sdk::TaskState::Running));
        if running {
            return fold(msgs);
        }
    }
...
fn key(chat: &mut ChatView, model: &Model, key: KeyEvent) -> Option<UiAction> {
    tui::chat::handle_chat_key(chat, model, key, (WIDTH, HEIGHT), at(NOW))
}
...
fn sdk_chat_enter_sends_the_draft() {
    ...
    tui::chat::handle_chat_paste(&mut chat, &model, "one more thing");
...
fn sdk_chat_ctrl_v_attaches_the_clipboard() {
    let model = session("text");
    let mut chat = open_chat(&model);
    tui::chat::handle_chat_clipboard(
        &mut chat,
        &model,
        tui::clipboard::ClipboardContent::Image {
