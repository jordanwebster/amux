...
fn banner_of(model: &Model, view: &ViewState) -> Option<String> {
    let _ = model;
    // The banner is the row directly under the header, in both chats.
    let frame = buffer_text(&render_buffer(model, view, Theme::default(), HEIGHT));
    let row = frame.lines().nth(1)?.trim().to_string();
...
fn a2a_inline_answer_dispatches_the_childs_own_command() {
    ...
    match action {
        Some(UiAction::Dispatch(ui_state::Command::Codex(ui_state::CodexCommand::Answer {
            agent,
            ..
...
fn a2a_inline_answer_matches_answering_from_the_childs_own_view() {
    ...
    let from_the_parent = tui::chat::handle_chat_key(
        chat,
        &model,
        press(KeyCode::Enter, KeyModifiers::NONE),
        (WIDTH, HEIGHT),
    ...
    let from_the_child = tui::chat::handle_chat_key(
        chat,
        &model,
        press(KeyCode::Enter, KeyModifiers::NONE),
        (WIDTH, HEIGHT),
...
fn a2a_inline_answer_works_the_other_way_round() {
    ...
    match action {
        Some(UiAction::Dispatch(ui_state::Command::Claude(
            ui_state::ClaudeCommand::AnswerAsk { agent, .. },
...
fn a2a_inline_answer_points_the_interrupt_at_the_agent_on_screen() {
    ...
    let mut view = docked(&model, LEAD);
    let chat = view.chat.as_mut().expect("an open chat");
    let while_docked = tui::chat::handle_chat_key(
        chat,
        &model,
    ...
    assert_eq!(
        while_docked,
        Some(UiAction::Dispatch(ui_state::Command::Codex(
            ui_state::CodexCommand::Interrupt {
    ...
    let mut view = chat_on(&model, LEAD);
    let chat = view.chat.as_mut().expect("an open chat");
    let plain = tui::chat::handle_chat_key(
        chat,
        &model,
        press(KeyCode::Char('x'), KeyModifiers::CONTROL),
        (WIDTH, HEIGHT),
    ...
    assert_eq!(
        plain,
        Some(UiAction::Dispatch(ui_state::Command::Claude(
            ui_state::ClaudeCommand::Interrupt {
                agent: agent_id(LEAD)
            }
        )))
    );
...
fn a2a_inline_answer_keeps_typing_out_of_the_parents_draft() {
    ...
    let mut view = docked(&model, LEAD);
    let chat = view.chat.as_mut().expect("an open chat");
    tui::chat::handle_chat_paste(chat, &model, "not for the lead");
    let chat = view.chat.as_mut().expect("an open chat");
    tui::chat::handle_chat_key(
        chat,
        &model,
...
fn a2a_inline_answer_hosts_a_session_childs_panel_in_a_codex_parents_chat() {
    ...
    match action {
        Some(UiAction::Dispatch(ui_state::Command::ClaudeSdk(
            ui_state::claude_sdk::ClaudeSdkCommand::AnswerAsk { agent, .. },
...
fn a2a_inline_answer_points_the_interrupt_at_a_session_child() {
    ...
    assert_eq!(
        action,
        Some(UiAction::Dispatch(ui_state::Command::ClaudeSdk(
            ui_state::claude_sdk::ClaudeSdkCommand::Interrupt {
