...
fn claude_sdk_ask_esc_steps_back_and_never_answers() {
    ...
    tui::chat::handle_chat_paste(&mut chat, &model, "write it elsewhere");
    assert!(
...
fn claude_sdk_ask_plan_opens_the_reader() {
    ...
    tui::chat::handle_chat_paste(&mut chat, &model, "cap it at four");
    assert_eq!(
        answered(press(&mut chat, &model, KeyCode::Enter)),
...
fn claude_sdk_ask_elicitation_forms_the_schema() {
    ...
    let mut chat = open_chat(&model);
    tui::chat::handle_chat_paste(&mut chat, &model, "PELICAN");
...
fn claude_sdk_ask_elicitation_orders_fields_by_name() {
    ...
    for value in ["kim", "chat", "0.4"] {
        tui::chat::handle_chat_paste(&mut chat, &model, value);
        assert!(press(&mut chat, &model, KeyCode::Enter).is_none());
    }
    assert_eq!(
        answered(press(&mut chat, &model, KeyCode::Enter)),
...
fn claude_sdk_ask_control_keys_reach_past_the_panel() {
    ...
    assert!(press(&mut chat, &model, KeyCode::Char('3')).is_none());
    assert!(press(&mut chat, &model, KeyCode::Enter).is_none());
    tui::chat::handle_chat_paste(&mut chat, &model, "not this file");
    assert!(
        key(
            &mut chat,
            &model,
