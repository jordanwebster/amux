...
use ui_state::codex::{CodexCommand, CodexDecision};
use ui_state::{
...
fn live_rows() -> Vec<Value> {
    vec![
        ...
        json!({"type":"item/started","item":{"id":"cmd-1","type":"commandExecution","command":"cargo test -p ui-state","cwd":"/work/amux","status":"inProgress"}}),
...
fn idle_rows() -> Vec<Value> {
    vec![
        ...
        json!({"type":"item/completed","item":{"id":"cmd-2","type":"commandExecution","command":"cargo test -p ui-state","cwd":"/work/amux","status":"completed","exitCode":0,"aggregatedOutput":"42 passed"}}),
...
fn attachment(
    kind: ui_state::ArtifactKind,
    name: &str,
    mime: &str,
    size: usize,
) -> ui_state::DraftAttachment {
    ui_state::DraftAttachment::from_bytes(kind, name, mime, vec![b'z'; size])
}

fn element(attachment: &ui_state::DraftAttachment) -> String {
...
fn refs_row(attachments: &[&ui_state::DraftAttachment]) -> Value {
    json!({
...
fn ctrl_x_dispatches_while_a_steer_is_in_flight_on_an_active_turn() {
    ...
    assert_eq!(
        ui_state::codex::send_gate(&in_flight, agent_id()),
        ui_state::codex::SendGate::InputInFlight
    );
...
fn working_line_advertises_only_the_actions_the_write_gate_allows() {
    ...
    assert!(!ui_state::codex::allows_steer(&in_flight, agent_id()));
    assert!(ui_state::codex::allows_interrupt(&in_flight, agent_id()));
...
fn refusing_approval_gates_remove_selection_and_confirm_affordances() {
    ...
    for (name, model) in [("stale", stale), ("replaying", replaying)] {
        ...
        assert!(
            !ui_state::codex::allows_answer(&model, agent_id()),
...
fn codex_keys_follow_the_write_gate_and_preserve_a_refused_steer_draft() {
    ...
    assert_eq!(
        ui_state::codex::send_gate(&stale, agent_id()),
        ui_state::codex::SendGate::Unknown
    );
    ...
    assert_eq!(
        ui_state::codex::send_gate(&replaying_approval, agent_id()),
