...
use ui_state::{
    AgentPhase, Attention, Command, DraftAttachment, InputPayload, Model, Msg, OpError, OpId,
    OpOutcome,
};
use uuid::Uuid;
...
async fn independent_runtimes_keep_account_identity_and_node_ownership_separate() {
    ...
    let personal_admin = installation.client(personal).unwrap();
    let work_admin = installation.client(work).unwrap();
    let personal_agent = create_test_agent(&personal_admin, dir.path()).await;
    let work_agent = create_test_agent(&work_admin, dir.path()).await;

    // Each view owns an independent client connection and reducer instance.
    // The account identity comes from that connection; there is no process-
    // global selected profile for either runtime to overwrite.
    let mut personal_view = Runtime::start_with_client(
        installation.client(personal).unwrap(),
        RuntimeOptions::default(),
    );
    let mut work_view = Runtime::start_with_client(
        installation.client(work).unwrap(),
        RuntimeOptions::default(),
    );
    wait_for(
        &mut personal_view,
        "personal view synchronization",
        |model| model.is_synchronized() && model.agent(personal_agent.id).is_some(),
    )
    .await;
    wait_for(&mut work_view, "work view synchronization", |model| {
        model.is_synchronized() && model.agent(work_agent.id).is_some()
    })
    .await;
    assert!(personal_view.model().agent(work_agent.id).is_none());
    assert!(work_view.model().agent(personal_agent.id).is_none());

    // Releasing one attached view releases only its client-side resources.
    // The externally owned installation and the other account remain live.
    drop(personal_view);
    let rename = work_view.dispatch(Command::RenameAgent {
        agent: work_agent.id,
        name: "work-still-live".to_string(),
    });
    wait_for(
        &mut work_view,
        "work rename after personal close",
        move |model| {
            model.finished_op(rename).is_some()
                && model
                    .agent(work_agent.id)
                    .and_then(|card| card.agent.name.as_deref())
                    == Some("work-still-live")
        },
    )
    .await;

    let personal_agents = personal_admin.list_agents().await.unwrap();
    assert_eq!(personal_agents.len(), 1);
    assert_eq!(personal_agents[0].id, personal_agent.id);
    assert_eq!(
        work_admin.list_agents().await.unwrap()[0].name.as_deref(),
        Some("work-still-live")
    );
...
async fn attaching_stores_the_picked_bytes_and_answers_with_what_a_token_names() {
    ...
    let outcome = ui_runtime::execute_put(&client, agent, picked.clone()).await;
    assert_eq!(
    ...
    assert!(matches!(
        ui_runtime::execute_put(&client, agent, too_large).await,
        OpOutcome::Error {
...
async fn attachments_open_uses_one_persistent_cache_and_refetches_tampering() {
    ...
    let opener = {
        ...
        Arc::new(move |meta: &artifacts::ArtifactMeta, path: &Path| {
            ...
            Ok(())
        }) as ui_runtime::AttachmentOpener
    };
...
async fn switcher_rejects_late_results() {
    ...
    assert_eq!(runtime.generation(), ui_runtime::Generation(1));
...
#[cfg(unix)]
fn socket_connector(socket: &Path) -> ui_runtime::Connector {
...
async fn claude_sdk_attachment_runtime_encodes_prompt_and_preserves_pins() {
    ...
    let outcome = execute_put_then_send(
        ...
        InputPayload::ClaudeSdk {
            payload: ui_state::claude_sdk::ClaudeSdkInput::Prompt {
                text: "look".into(),
            },
        },
        vec![draft.id.clone()],
