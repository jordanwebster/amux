...
async fn a_removed_account_leaves_nothing_of_its_profile_behind() {
    ...
    for profile in [kept, removed] {
        let path = app_runtime::cache::store_path(&cache, &profile.to_string());
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        store::Store::open(&path).await.unwrap().close().await;
        let quarantine = path.parent().unwrap().join("quarantine/previous");
        std::fs::create_dir_all(&quarantine).unwrap();
        std::fs::write(quarantine.join("store.sqlite"), b"retained corrupt cache").unwrap();
        assert!(path.is_file());
        assert!(quarantine.join("store.sqlite").is_file());
    ...
    assert!(!app_runtime::cache::store_path(&cache, &removed.to_string()).exists());
    assert!(
        !app_runtime::cache::store_path(&cache, &removed.to_string())
            .parent()
            .unwrap()
            .join("quarantine/previous/store.sqlite")
            .exists()
    );
    ...
    assert!(app_runtime::cache::store_path(&cache, &kept.to_string()).exists());
    assert!(
        app_runtime::cache::store_path(&cache, &kept.to_string())
            .parent()
            .unwrap()
            .join("quarantine/previous/store.sqlite")
            .is_file()
...
async fn caches_that_outlived_their_profile_are_still_deleted() {
    ...
    .unwrap();
    let path = app_runtime::cache::store_path(&cache, &orphan.to_string());
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
    store::Store::open(&path).await.unwrap().close().await;
    assert!(path.is_file());
    ...
    assert_eq!(embedded.forgotten, ["work"]);
    assert!(!app_runtime::cache::store_path(&cache, &orphan.to_string()).exists());
