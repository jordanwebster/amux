...
fn banner_line(model: &Model, width: usize, theme: Theme) -> Line<'static> {
    ...
    let away = model
        .hosts()
        .filter(|host| host.entry.trust_status == amux_ui::HostTrustStatus::Trusted)
    ...
    let signed_out_with_offline = matches!(
        model.cloud_state(),
        amux_ui::CloudState::SignedOut | amux_ui::CloudState::AuthRequired
    ) && model.hosts().any(|host| {
        host.entry.trust_status == amux_ui::HostTrustStatus::Trusted
            && host.entry.via == amux_ui::HostVia::Offline
    });
