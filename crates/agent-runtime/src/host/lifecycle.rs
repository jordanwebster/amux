...
pub(crate) fn publish_activity(state: &mut AgentServiceState, host_id: Uuid) {
    ...
    for context in state.local_agents.values_mut() {
        let last_activity = context.last_activity();
        if last_activity > context.published_activity {
            context.published_activity = last_activity;
            updated.push(context.record(host_id).agent_updated_event());
        }
    }
...
fn commit_local_name_update(
    ...
    // Announce the registry's whole record rather than the session's: the
    // session knows nothing of the agent's status or remembered activity.
    let announced = context.record(updated.host_id);
    state
        .local_agent_events
        .emit(announced.agent_updated_event());
    Ok(())
}
...
mod tests {
    ...
    fn test_state() -> SharedAgentServiceState {
