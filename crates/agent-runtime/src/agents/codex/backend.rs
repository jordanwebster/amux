        ...
        match input {
            ...
            CodexSdkV1Input::SetPreset { approval, sandbox } => {
                ...
                let sandbox = match sandbox {
                    model::SandboxPolicy::ReadOnly => json!({"type":"readOnly"}),
                    model::SandboxPolicy::WorkspaceWrite => json!({"type":"workspaceWrite"}),
                    model::SandboxPolicy::DangerFullAccess => {
...
mod tests {
    ...
    async fn start_delivery_replay(
        ...
        let fixture = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../codex-specs/fixtures/codex")
            .join(fixture);
    ...
    async fn a2a_codex_deliver_idle_injects_then_starts_empty_turn() {
        ...
        let tagged = model::envelope::format(&envelope);
    ...
    async fn a2a_codex_completion_replays_last_agent_message() {
        let fixture = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../codex-specs/fixtures/codex/a2a_last_message.io.jsonl");
        let mut script = load_script(fixture);
