...
fn cache_path(cache_dir: &std::path::Path, account: &str) -> std::path::PathBuf {
    ...
    app_runtime::cache::store_path(cache_dir, &profile.to_string())
}
...
async fn mobile_lifecycle_connects_reconnects_and_stops_at_the_c_boundary() {
    ...
    let mut seed = Embedded::open(&parsed, requests).await.unwrap();
    tokio::time::timeout(Duration::from_secs(10), async {
        while *seed.sessions.relay.borrow_and_update() != RelayConnection::Connected {
            seed.sessions.relay.changed().await.unwrap();
        }
    })
    .await
    .unwrap();
    ...
    seed.sessions
        ...
        .unwrap();
    let agent = admin
        .create_agent(model::CreateAgentRequest {
            agent_id: host.host_id(),
            host_id: None,
            name: Some("mobile-lifecycle-agent".into()),
            agent_type: model::AgentType::TestAgent {
                command: "cat".into(),
            },
...
fn the_phones_agent_writes_decode_as_the_commands_they_name() {
    ...
    assert_eq!(
        ...
        Command::SetPreset {
            agent: id,
            approval: model::ApprovalPolicy::Never,
            sandbox: model::SandboxPolicy::DangerFullAccess,
        }
    );
    assert_eq!(
        decoded(
...
fn a_picked_file_becomes_a_store_of_exactly_the_bytes_that_were_picked() {
    ...
    let Some(Command::PutAttachment { agent, attachment }) = picked_command(
    ...
    assert_eq!(agent, id);
    assert_eq!(attachment.kind, ui_state::ArtifactKind::Image);
    ...
    assert_eq!(
        attachment.id,
        ui_state::DraftAttachment::from_bytes(
            ui_state::ArtifactKind::File,
            "renamed.bin",
    ...
    assert_eq!(file.kind, ui_state::ArtifactKind::File);
    ...
    assert!(picked_command("{\"agent\":\"not-a-uuid\"}", Vec::new()).is_none());
    assert!(
        picked_command(
...
fn build_marker_names_the_debug_tools_library() {
    ...
    let build = unsafe { std::ffi::CStr::from_ptr(crate::amux_app_build()) }
        .to_str()
        .unwrap();
    let version = unsafe { std::ffi::CStr::from_ptr(crate::amux_app_version()) }
        .to_str()
        .unwrap();
...
fn mobile_lifecycle_rejects_invalid_endpoints_and_config() {
    ...
    assert!(start(&override_cloud, &events).is_null());
    unsafe {
        assert!(amux_app_start(std::ptr::null(), capture, std::ptr::null_mut()).is_null());
        amux_app_stop(std::ptr::null_mut());
    }
}
    ...
    let mut ids = std::collections::BTreeSet::new();
    {
        ...
        for index in 0..150 {
            let command = if index % 2 == 0 {
                ...
                serde_json::to_string(&Command::Claude(ui_state::ClaudeCommand::SendPrompt {
            ...
            let id = unsafe { amux_app_dispatch(running.handle, invalid.as_ptr()) };
            assert!(!id.is_null());
        ...
        .unwrap();
        let id = unsafe { amux_app_dispatch(running.handle, subscribe.as_ptr()) };
        unsafe {
            amux_app_free(id);
        }
    }
...
async fn mobile_unpaired_relay_hosts_are_discovered_without_entering_the_fleet() {
    ...
    for reconnect in [false, true] {
        if reconnect {
            ...
            tokio::time::timeout(Duration::from_secs(10), async {
                loop {
                    let snapshot = owned_json(unsafe { amux_app_snapshot(running.handle) });
...
async fn mobile_cache_offline_restart_reconciles_in_place_and_exports_report() {
    ...
    let report = owned_json(unsafe { amux_app_report_snapshot(running.handle) });
    assert_eq!(
    ...
    let replay = ui_runtime::replay_msgs(&replay_path).unwrap();
    ...
    let missing = owned_json(unsafe {
        amux_app_replay_report(CString::new("/nonexistent/msgs.jsonl").unwrap().as_ptr())
    });
    assert!(missing["error"].is_string(), "{missing}");
    assert!(unsafe { amux_app_replay_report(std::ptr::null()) }.is_null());
    let snapshot: ui_state::Model =
        serde_json::from_value(owned_json(unsafe { amux_app_snapshot(running.handle) })).unwrap();
    assert_eq!(
...
fn a_seeded_store_is_what_a_launch_and_a_cached_conversation_read() {
    ...
    let remembered = json!({
        ...
        "agents": [{"id": agent, "host_id": host, "name": "kept", "command": "claude",
                    ...
                    "readonly": false, "args": [], "created_at": "2026-09-16T12:00:00.000Z", "last_activity": "2026-09-16T12:00:00.000Z"}],
...
fn a_seeded_standing_is_what_a_launch_draws_for_each_remembered_agent() {
    ...
    let agent = |id: u128, host: uuid::Uuid| {
        json!({"id": uuid::Uuid::from_u128(id), "host_id": host, "name": format!("agent-{id}"),
               ...
               "created_at": "2026-09-13T09:00:00Z", "last_activity": "2026-09-13T09:00:00Z"})
    };
    let remembered = json!({
        "local": uuid::Uuid::from_u128(9),
        "hosts": [
...
fn mobile_cache_missing_is_empty_but_unusable_stores_report_the_remedy() {
    let root = test_root();
    app_runtime::cache::remember_profiles(
        root.path(),
        &[("personal".into(), model::ProfileId::new())].into(),
    )
    .unwrap();
...
async fn phone_first_frame_reclaims_a_store_over_its_soft_budget() {
    let root = test_root();
    let config = config(root.path(), "http://127.0.0.1:9".into(), json!("Callback"));
    let (requests, _requests_rx) = mpsc::channel(8);
    let mut embedded =
        app_embedded::Embedded::open(&serde_json::from_value(config.clone()).unwrap(), requests)
            .await
            .unwrap();
    embedded.shutdown().await;
    let path = cache_path(root.path().join("cache").as_path(), "personal");
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
...
mod mobile_cache_authoritative_inventory_prunes_offline_deletions_and_unpairing {
    ...
    async fn check(scenario: &str) {
        ...
        match scenario {
            ...
            "unpair" => {
                ...
                offline
                    .sessions
                    .admin
                    .unpair(host.host_id(), "Remove this device".into())
                    .await
                    .unwrap();
        ...
        tokio::time::timeout(Duration::from_secs(5), async {
            loop {
                let model: ui_state::Model = serde_json::from_value(owned_json(unsafe {
...
async fn mobile_pairing_over_the_relay_admits_the_hosts_agents_to_the_fleet() {
    ...
    assert_ne!(
        start_pairing.cloud_url.as_deref(),
        Some(format!("http://{}", net.relay_addr()).as_str()),
    ...
    let client::PairingSecret::QrSecret(secret) = &start_pairing.secret else {
        panic!("QR pairing returned a PIN")
    };
    let payload =
        CString::new(node::encode_qr_pairing_payload(&start_pairing, secret).unwrap()).unwrap();
    let paired = owned_json(unsafe { amux_app_pair_qr(running.handle, payload.as_ptr()) });
    assert_eq!(
    ...
    let refused = owned_json(unsafe { amux_app_pair_qr(running.handle, nonsense.as_ptr()) });
    assert!(
...
async fn mobile_unsubscribe_releases_the_stream_a_closed_conversation_asked_for() {
    ...
    let dispatch = |command: &str, agent: uuid::Uuid| {
        ...
        let id = unsafe { amux_app_dispatch(running.handle, json.as_ptr()) };
    ...
    let later = owned_json(unsafe { amux_app_snapshot(running.handle) });
    assert!(
...
async fn mobile_pairing_by_code_writes_no_trust_until_it_is_confirmed() {
    ...
    let started = host
        ...
        .await
        .unwrap();
    let client::PairingSecret::Pin(pin) = started.secret else {
...
async fn mobile_pairing_by_link_authenticates_against_the_configured_cloud() {
    ...
    let client::PairingSecret::QrSecret(secret) = &offer.secret else {
        panic!("QR pairing returned a PIN")
    };
    assert_eq!(offer.cloud_url.as_deref(), Some("https://amux.sh"));
    assert_ne!(
        offer.cloud_url.as_deref(),
        Some(format!("http://{}", net.relay_addr()).as_str())
    );
    let payload = node::encode_qr_pairing_payload(&offer, secret).unwrap();
    ...
    let other_offer = other_host
        ...
        .await
        .unwrap();
    let client::PairingSecret::QrSecret(other_secret) = &other_offer.secret else {
        panic!("QR pairing returned a PIN")
    };
    let elsewhere = node::encode_qr_pairing_payload(&other_offer, other_secret).unwrap();
    let wrong = answered(
        &mut receive,
...
async fn mobile_retry_now_shortens_a_live_connections_wait() {
    ...
    async fn disconnected(relay: &mut tokio::sync::watch::Receiver<model::RelayConnection>) {
        tokio::time::timeout(Duration::from_secs(20), async {
            loop {
                relay.changed().await.unwrap();
                if matches!(
                    *relay.borrow_and_update(),
                    model::RelayConnection::Disconnected { .. }
                ) {
                    break;
                }
            }
    ...
    .unwrap();
    let (requests, _receive) = mpsc::channel(1);
    let runtime = Embedded::open(&config, requests).await.unwrap();
    let mut relay = runtime.sessions.relay.clone();
    tokio::time::timeout(Duration::from_secs(20), async {
        while *relay.borrow_and_update() != model::RelayConnection::Connected {
            relay.changed().await.unwrap();
        }
    })
    .await
    ...
    let settled = runtime.sessions.link.attempts();
...
async fn mobile_retry_now_reaches_the_connection_through_the_bridge() {
    ...
    let id = unsafe { amux_app_dispatch(running.handle, command.as_ptr()) };
...
async fn mobile_going_away_releases_the_link_and_coming_back_reconciles() {
    ...
    unsafe { amux_app_set_active(handle, false) };
    ...
    unsafe { amux_app_set_active(handle, true) };
    until(&mut receive, handle, &token, |event| {
...
async fn seed_agent(host: &testnet::Daemon, id: u128, name: &str) -> client::Client {
    let admin = host.admin_client().await;
    admin
        .create_agent(model::CreateAgentRequest {
            agent_id: uuid::Uuid::from_u128(id),
...
async fn pair_with(handle: *mut Handle, host: &testnet::Daemon) -> Value {
    let start_pairing = host.pairing_admin().await.start_qr_pairing().await.unwrap();
    let client::PairingSecret::QrSecret(secret) = &start_pairing.secret else {
        panic!("QR pairing returned a PIN")
    };
    let payload =
        CString::new(node::encode_qr_pairing_payload(&start_pairing, secret).unwrap()).unwrap();
    owned_json(unsafe { amux_app_pair_qr(handle, payload.as_ptr()) })
}
...
async fn select_account(
    ...
    let reply = unsafe { amux_app_dispatch(handle, json.as_ptr()) };
    ...
    let profile_files = |root: &std::path::Path| {
        ...
        for entry in std::fs::read_dir(root.join("data/installation/profiles")).unwrap() {
            ...
            for file in ["data/device.key", "data/trust.json", "data/host_id"] {
                ...
                if file == "data/trust.json" {
                    // A trust entry also caches the last account-binding fact
                    // its peer announced. That is an observation, not part of
                    // the trust, and a peer that reconnects may announce it
                    // when it had not before. Compare what pairing decided.
                    let mut json: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
                    for entry in json.as_object_mut().unwrap().values_mut() {
                        entry.as_object_mut().unwrap().remove("signed_in");
                    }
                    bytes = serde_json::to_vec(&json).unwrap();
                }
                files.insert((entry.file_name(), file), bytes);
            }
        }
        files
    };
...
async fn mobile_profiles_switching_drops_every_late_result_from_the_previous_account() {
    ...
    let unknown = owned_json(unsafe { amux_app_late_results(handle) });
    assert_eq!(unknown["dropped"], 0, "{unknown}");

    select_account(&mut receive, handle, "work").await;
    let reported = owned_json(unsafe { amux_app_late_from_previous(handle) });
    assert_eq!(
...
async fn mobile_profiles_the_remembered_fleet_belongs_to_the_account_that_saw_it() {
    ...
    .await;
    // A live frame precedes its asynchronous store write. Establish the
    // remembered state before retiring the account's store worker.
    stored(&cache_dir, "personal", |e| {
        agent_names(e) == ["fix-login"] && host_names(e) == ["workstation"]
    })
    .await;
...
async fn mobile_profiles_report_what_is_waiting_on_the_account_that_is_not_on_screen() {
    ...
    .unwrap();
    let reply = unsafe { amux_app_dispatch(handle, subscribe.as_ptr()) };
...
/// Every handle ends exactly once, however it ends.
///
/// A start that cannot open its installation reports one terminal connection
/// event and nothing after it; a start that is stopped before it connected,
/// and one stopped while events are in flight, deliver nothing after stop
/// returns; and the same data directory starts again as many times as asked.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn every_handle_ends_exactly_once_however_it_is_stopped() {
    let net = TestNet::builder()
        .cloud()
        .daemon("workstation")
        .cloud_only()
    ...
    for round in 0..3 {
        ...
        assert!(!running.handle.is_null(), "round {round} did not start");
        until(&mut receive, running.handle, &token, |e| {
            e["Connection"]["state"] == "connected"
        })
        .await;
        ...
        assert_eq!(
            events.captured.lock().unwrap().len(),
            count,
            "round {round}: a callback arrived after stop returned"
        );
        let stopped = events
            .captured
            .lock()
            .unwrap()
            .iter()
            .filter(|e| e["Connection"]["reason"] == "stopped")
            .count();
        assert_eq!(
            stopped, 0,
            "round {round}: a clean stop announced itself as a failure"
        );
    }

    // Stopped before it ever connected: still nothing after stop returns.
    {
        let (sender, _receive) = mpsc::unbounded_channel();
        let events = Events {
            sender,
        ...
        assert_eq!(
            events.captured.lock().unwrap().len(),
            count,
            "a callback arrived after an early stop returned"
        );
    }

    // A failure during startup: the installation root is under a file, so
    // it cannot be created. Exactly one terminal connection event, then
    // silence, and stop is still safe to call.
    {
        let blocked = root.path().join("blocked");
        std::fs::write(&blocked, b"not a directory").unwrap();
        let mut failing = config.clone();
        failing["data_dir"] = json!(blocked.join("data"));
        let (sender, mut receive) = mpsc::unbounded_channel();
        let events = Events {
            sender,
            captured: Mutex::new(vec![]),
        ...
        let running = Running {
            handle: start(&failing, &events),
            _events: &events,
        };
        assert!(
            !running.handle.is_null(),
            "a start that fails later still starts"
        );
        tokio::time::timeout(Duration::from_secs(30), async {
            loop {
                let event = receive.recv().await.expect("callback channel closed");
                if event["Connection"]["reason"] == "stopped" {
                    break;
                }
            }
        })
        .await
        .expect("a failed start never reported the worker stopped");
        tokio::time::sleep(Duration::from_millis(350)).await;
        drop(running);
        tokio::time::sleep(Duration::from_millis(350)).await;
        let captured = events.captured.lock().unwrap().clone();
        ...
        assert_eq!(
            terminal, 1,
            "a failed start ended more than once: {captured:?}"
        );
        assert!(
            captured.iter().any(|e| e.get("Invariant").is_some()),
            "the failure was not written out as a diagnostic: {captured:?}"
        );
        assert_eq!(
            captured.last().unwrap()["Connection"]["reason"],
            "stopped",
...
async fn signed_out_starts_on_one_unbound_profile_and_pairs_on_this_network() {
    ...
    assert!(
        app_runtime::cache::store_path(&cache_dir, &unbound).is_file(),
