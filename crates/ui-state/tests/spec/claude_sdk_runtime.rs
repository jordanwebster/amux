...
fn claude_sdk_subscription_policy_opens_only_requested_conversations() {
    ...
    assert!(ui_state::update(&mut model, agent_up(&sdk(AGENT, "nova"))).is_empty());
}
fn attachments() -> Vec<Msg> {
    let draft = ui_state::DraftAttachment::from_bytes(
        ArtifactKind::File,
        "facts.txt",
        "text/plain",
        b"facts".to_vec(),
...
fn claude_sdk_attachments_keep_pins_metadata_and_fetched_diffs() {
    ...
    assert!(effects.iter().any(|effect| matches!(effect,
        Effect::PutThenSend { input:ui_state::InputPayload::ClaudeSdk { payload:ui_state::claude_sdk::ClaudeSdkInput::Prompt { text } }, pin, puts, .. }
...
fn claude_sdk_family_needs_tracks_asks_and_offline_hosts() {
    ...
    host.online = false;
    ui_state::update(&mut model, host_up(&host));
...
fn claude_sdk_initialized_commands_reach_provider_facts_without_a_turn() {
    ...
    let provider = ui_state::provider::facts(&model, agent_id(AGENT));
    assert_eq!(
        provider
            .commands
            .iter()
...
fn claude_sdk_session_facts_survive_feed_rows_and_clear_stale_context() {
    ...
    assert!(
        provider
            .commands
            .iter()
            .all(|command| command.source == ui_state::provider::CommandSource::Claude)
    );
    assert_eq!(
        provider
            .commands
    ...
    assert_eq!(
        ui_state::provider::facts(&tail, agent_id(AGENT)).commands,
        provider.commands
    );
    ui_state::update(
        &mut model,
...
fn claude_sdk_runtime_sequences_replay_and_capture_public_state() {
    for (name, msgs) in sequences() {
        let mut live = ui_state::Model::default();
        let mut recorded = Vec::new();
        for msg in msgs.clone() {
            recorded.push(serde_json::to_string(&msg).unwrap());
            ui_state::update(&mut live, msg);
            assert!(live.check_invariants().is_empty(), "{name}");
        }
        let mut replayed = ui_state::Model::default();
        for row in recorded {
            ui_state::update(&mut replayed, serde_json::from_str(&row).unwrap());
        }
        ...
        if let Some(path) = std::env::var_os("CLAUDE_SDK_RUNTIME_EVIDENCE") {
            ...
            let capture = json!({"session":model.claude_sdk(agent_id(AGENT)).map(|l|l.session()),
                "provider": ui_state::provider::facts(&model, agent_id(AGENT)),
