...
use serde_json::json;
use ui_state::codex::{CodexCommand, CodexDecision, CodexPhase, SendGate};
use ui_state::{
...
fn projections(model: &Model) -> (CodexPhase, Attention, SendGate) {
    ...
    (
        ui_state::codex::phase(model, agent),
        card.attention,
        ui_state::codex::send_gate(model, agent),
    )
}
...
fn active_turn_with_input_in_flight_keeps_only_interrupt_safe() {
    ...
    assert!(!ui_state::codex::allows_prompt(&model, agent));
    assert!(!ui_state::codex::allows_steer(&model, agent));
    assert!(!ui_state::codex::allows_answer(&model, agent));
    assert!(ui_state::codex::allows_interrupt(&model, agent));
}
...
fn observation_only_preserves_codex_projections_but_refuses_every_action() {
    ...
    for (writable_name, observer_name) in pairs {
        ...
        assert_eq!(
            ui_state::codex::send_gate(&observer, agent),
            SendGate::ObserverReadOnly
        );
        assert!(!ui_state::codex::allows_prompt(&observer, agent));
        assert!(!ui_state::codex::allows_steer(&observer, agent));
        assert!(!ui_state::codex::allows_interrupt(&observer, agent));
        assert!(!ui_state::codex::allows_answer(&observer, agent));
    ...
    assert!(!ui_state::codex::allows_interrupt(&input_observer, agent));
...
fn reconnect_readonly_remains_distinct_and_outranks_observer_policy() {
    ...
    assert_eq!(
        ui_state::codex::send_gate(&model, agent).refusal(),
