...
use serde_json::json;
use ui_state::claude::{AskWhy, ChatPhase, PhaseTag};
use ui_state::{
...
fn observation_only(mut messages: Vec<Msg>) -> Vec<Msg> {
    for message in &mut messages {
        if let Msg::Server(ui_state::ServerMsg::AgentUpserted { agent }) = message {
            agent.readonly = true;
        }
    }
    messages
...
fn observation_only_preserves_claude_projections_but_refuses_interaction() {
    ...
    for (writable_name, observer_name) in pairs {
        ...
        assert!(!ui_state::claude::allows_answer(&observer, agent));
        assert!(!ui_state::claude::allows_interrupt(&observer, agent));
        assert_eq!(
            ui_state::claude::mode_cycle_gate(&observer, agent),
...
fn phase_attention_and_gates_agree_after_every_message() {
    for (name, messages) in agreement_cases() {
        ...
        for (step, message) in messages.into_iter().enumerate() {
            ui_state::update(&mut model, message);
...
fn claude_specific_exceptions_and_send_precedence_are_explicit() {
    ...
    assert!(matches!(
        ui_state::claude::phase(&offline, agent_id(AGENT)),
        ChatPhase::Idle { .. }
    ));
    assert_eq!(
...
fn prompt_echo_ages_from_dispatch_while_its_send_gate_stays_closed() {
    ...
    ui_state::update(&mut model, send_prompt(41, "next task"));
    assert_eq!(
    ...
    ui_state::update(&mut model, tick(10 + 601 + 601));
    assert_eq!(
        projections(&model).map(|(_, attention, gate)| (attention, gate)),
    ...
    ui_state::update(&mut model, tick(10 + 601 + 602 + 601));
    assert_eq!(
        projections(&model),
        Some((ChatPhase::Unknown, Attention::Unknown, SendGate::Unknown)),
    ...
    ui_state::update(&mut failed, op_failed(op(42), "send rejected"));
...
fn echo_free_working_still_ages_from_transcript_delivery() {
    ...
    ui_state::update(&mut model, tick(10 + 600));
    assert_eq!(
        projections(&model),
        Some((ChatPhase::Working, Attention::Working, SendGate::Working))
    );

    ui_state::update(&mut model, tick(10 + 601));
    assert_eq!(
        projections(&model),
        Some((ChatPhase::Unknown, Attention::Unknown, SendGate::Unknown))
    );
