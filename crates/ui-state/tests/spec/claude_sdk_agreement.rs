...
fn exited_family(newer: &str) -> Vec<Msg> {
    ...
    for name in [AGENT, "pty-agreement"] {
        ...
        if name == AGENT {
            child.kind = model::AgentKind::Claude {
...
pub fn sequences() -> Vec<(&'static str, Vec<Msg>)> {
    ...
    for msg in &mut readonly {
        if let Msg::Server(ui_state::ServerMsg::AgentUpserted { agent }) = msg {
            agent.readonly = true;
        }
    }
...
fn exited_claude_backends_share_badge_rank_and_clear_family_needs() {
    for newer in [AGENT, "pty-agreement"] {
        ...
        assert_eq!(
            ui_state::claude::send_gate(&model, pty.agent.id),
            ui_state::claude::SendGate::Exited
        );
...
fn invariant_detects_kernel_exit_disagreeing_with_a_working_cached_card() {
    ...
    assert!(
        violations
            .iter()
            .any(|v| matches!(v, ui_state::Violation::ClaudeSdkProjection { .. }))
    );
    assert!(
        !violations
            .iter()
            .any(|v| matches!(v, ui_state::Violation::AttentionMismatch { .. })),
