...
fn draft(text: &str) -> Draft {
    Draft {
        segments: vec![ui_state::DraftSegment::Text { text: text.into() }],
        attachments: vec![],
    }
}
...
fn queue_hold_delivers_one_native_input_at_turn_end() {
    for codex in [false, true] {
        ...
        assert!(
            ui_state::queue::can_hold(&model, agent_id(AGENT)),
            "{model:?}"
        );
        ...
        match &effects[0] {
            ...
            Effect::SendInput {
                ...
                assert_eq!(*sent, op(1));
                let ui_state::CodexInput::UserTurn { input } = payload else {
                    panic!("new user turn");
                };
                assert_eq!(
...
fn queue_interrupt_keeps_the_held_message() {
    for codex in [false, true] {
        ...
        let input = if codex {
            Command::Codex(ui_state::CodexCommand::Interrupt {
                agent: agent_id(AGENT),
            })
        } else {
            Command::Claude(ui_state::ClaudeCommand::Interrupt {
                agent: agent_id(AGENT),
            })
        };
...
fn queue_offline_cancel_and_agent_removal_never_send() {
    for codex in [false, true] {
        let mut model = fold(seq([
            ...
            vec![
                hold(1, "keep me"),
                disconnected(ui_state::DisconnectReason::ApplicationShutdown),
            ],
        ]));
...
fn queue_attachments_replay_and_cancel_losslessly() {
    ...
    let text = ui_state::format_mention(&ui_state::Mention {
        kind: ui_state::MentionKind::File {
