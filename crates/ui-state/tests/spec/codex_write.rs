...
fn failure_message(model: &ui_state::Model, op_n: u8) -> String {
...
fn complete_replay_stays_non_idle_everywhere_until_the_ready_row_arrives() {
    ...
    assert_eq!(ui_state::codex::phase(&model, agent), CodexPhase::Replaying);
...
fn initial_reconnect_failure_outranks_the_missing_ready_row() {
    ...
    assert_eq!(ui_state::codex::phase(&model, agent), CodexPhase::ReadOnly);
    assert_eq!(
        ui_state::codex::send_gate(&model, agent),
        SendGate::ReadOnly
    );
...
fn stale_turn_id_cannot_bypass_the_authoritative_write_gate() {
    ...
    assert_eq!(
        ui_state::codex::send_gate(&fold(base.clone()), agent_id(AGENT)),
        SendGate::Unknown
    );
...
fn observation_only_refuses_every_codex_action_before_local_mutation() {
    ...
    for (op_n, rows, native) in cases {
        ...
        let before_phase = ui_state::codex::phase(&before, agent_id(AGENT));
        ...
        assert_eq!(
            ui_state::codex::send_gate(&model, agent_id(AGENT)),
