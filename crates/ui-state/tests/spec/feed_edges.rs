...
use serde_json::json;
use ui_state::Msg;
use ui_state::claude::{FeedEntryKind, SuccessFacts, ToolInvocation, ToolOutcome, TurnDuration};
...
fn finality_of(model: &ui_state::Model, id: &str) -> ui_state::claude::MessageFinality {
...
fn meta_rows_are_inert_in_either_content_form() {
    ...
    assert_eq!(
        ...
        ui_state::claude::MessageFinality::Open,
...
fn an_api_error_closes_the_open_message_as_abandoned() {
    ...
    assert_eq!(
        finality_of(&model, "msg_open"),
        ui_state::claude::MessageFinality::Abandoned
    );
    assert!(
        claude_layer(&model, "fix-auth-bug")
            .entries()
