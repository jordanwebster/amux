...
use ui_state::claude::{AskWhy, ChatPhase};
use ui_state::{Attention, Effect, FleetItem, Model, Msg, StreamCloseReason, StreamMsg, Why};
...
fn an_api_error_degrades_attention_to_unknown() {
    ...
    assert_eq!(
        ui_state::claude::phase(&model, agent_id("fix-auth-bug")),
        ChatPhase::Errored,
...
fn fleet_attention_and_chat_phase_share_one_interpretation() {
    for fixture in [
        ...
        for (step, row) in chat_rows(fixture).into_iter().enumerate() {
            ...
            let phase_needs = match ui_state::claude::phase(&model, agent_id("fix-auth-bug")) {
