...
use ui_state::codex::{CodexInput, SendGate};
use ui_state::provider::{ApprovalPolicy, SandboxPolicy, SettingsGate, facts, settings_gate};
use ui_state::{Command, Effect, InputPayload, Model, Msg, OpOutcome, StreamMsg, update};
...
fn model_effort_unknown_choices_and_stale_sessions_never_dispatch() {
    ...
    update(
        ...
        op_result(
            op(1),
            OpOutcome::Error {
                error: ui_state::OpError::general("transport failed"),
            },
        ),
    );
    assert_eq!(settings_gate(&model, agent_id(AGENT)), SettingsGate::Ready);
