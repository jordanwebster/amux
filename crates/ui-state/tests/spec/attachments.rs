...
fn review_element() -> String {
    format_mention(&Mention {
        kind: MentionKind::Review {
            ...
            comments: vec![
                ui_state::review::ReviewComment {
                    path: "src/main.rs".into(),
                    start_side: ui_state::review::Side::Old,
                    start_line: 3,
                    side: ui_state::review::Side::New,
                    line: 4,
                ...
                ui_state::review::ReviewComment {
                    ...
                    start_side: ui_state::review::Side::New,
                    start_line: 9,
                    side: ui_state::review::Side::New,
                    line: 9,
...
fn draft_attachments() -> Vec<ui_state::DraftAttachment> {
    vec![
...
fn attachments_fetched_diffs_have_an_independent_count_bound() {
    ...
    let ids: Vec<_> = (0..=ui_state::attachments::FETCHED_DIFFS_RETAINED)
...
fn attachments_recorder_keeps_identity_and_size_but_redacts_bytes() {
    ...
    let recorded_line = serde_json::to_string(&msg).expect("recorded JSON");
    let recorded: Value = serde_json::from_str(&recorded_line).expect("recorded JSON");
    ...
    assert!(
        !recorded_line.contains("recorder must not keep these image bytes"),
        "payload bytes never enter the recorder line"
    );
    let replayed: Msg = serde_json::from_str(&recorded_line).expect("redacted Msg replays");
    let Msg::Command {
