...
use serde_json::json;
use ui_state::claude::ClaudeLayer;
use ui_state::codex::CodexLayer;
use ui_state::{
...
fn claude_row(kind: model::envelope::EnvelopeKind, text: &str) -> serde_json::Value {
    ...
    json!({
        ...
        "origin": {"kind": "human"},
        "promptSource": "typed",
        "message": {"role": "user", "content": model::envelope::format(&envelope)},
    })
}
...
fn claude_sequence() -> Vec<Msg> {
    seq([
        ...
        vec![batch(
            ...
            10,
            vec![
                claude_row(model::envelope::EnvelopeKind::Message, "how far along?"),
                claude_row(model::envelope::EnvelopeKind::Completed, REPORT),
                claude_row(model::envelope::EnvelopeKind::Exited, ""),
            ],
        )],
    ])
}
...
fn claude_messages(layer: &ClaudeLayer) -> Vec<(AgentMessageKind, String)> {
    layer
        ...
        .filter_map(|entry| match &entry.kind {
            ui_state::claude::FeedEntryKind::AgentMessage(message) => {
...
fn codex_messages(layer: &CodexLayer) -> Vec<(AgentMessageKind, String)> {
    layer
        .entries()
        .filter_map(|entry| match &entry.kind {
            ui_state::codex::FeedEntryKind::AgentMessage(message) => {
                Some((message.kind.clone(), message.text.clone()))
            }
            _ => None,
        })
...
fn a2a_completed_row_projects_foldable_completion_per_layer() {
    ...
    let single_claude = fold(seq([
        ...
        vec![batch(
            CLAUDE_AGENT,
            10,
            vec![claude_row(model::envelope::EnvelopeKind::Completed, "done")],
        )],
    ]));
