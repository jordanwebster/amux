...
use ui_state::claude::{AgentMessageEntry, ClaudeLayer, FeedEntryKind};
use ui_state::{AgentMessageKind, Model, Msg};
...
fn a2a_claude_inbound_still_opens_the_turn_the_delivery_started() {
    let pasted = fold(seq([
        ...
        vec![batch(
            ...
            10,
            vec![user_row(
                &model::envelope::format(&formatted(model::envelope::EnvelopeKind::Message, None)),
                false,
            )],
        )],
    ]));
    assert_eq!(
        ui_state::claude::phase(&pasted, agent_id(AGENT)),
    ...
    let posted = fold(seq([
        ...
        vec![batch(
            ...
            10,
            vec![user_row(
                &model::envelope::format_cross_session(
                    &formatted(model::envelope::EnvelopeKind::Message, None),
                    "prompting",
                )
    ...
    assert_ne!(
        ui_state::claude::phase(&posted, agent_id(AGENT)),
        ui_state::claude::ChatPhase::Working,
    );
}
...
fn a2a_claude_inbound_agrees_with_the_daemon_formatter() {
    ...
    let carriers = [
        user_row(&model::envelope::format(&envelope), false),
        user_row(
