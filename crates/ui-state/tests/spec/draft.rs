...
use model::id_of;
use serde_json::json;
use ui_state::attachments::{Mention, MentionKind};
use ui_state::{ArtifactKind, Command, DraftAttachment, Effect, Msg, OpId, format_mention};
...
fn put_then_send(effects: &[Effect]) -> (&Vec<DraftAttachment>, &Vec<ui_state::ArtifactId>) {
...
fn draft_waits_for_a_takeover_and_sends_unchanged_after_it() {
    ...
    assert_eq!(
        ui_state::claude::send_gate(&taken_over, agent_id(AGENT)).refusal(),
    ...
    assert_eq!(
        ui_state::claude::send_gate(&returned, agent_id(AGENT)).refusal(),
        None,
