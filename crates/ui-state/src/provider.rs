...
pub(crate) fn update_settings(
    ...
    let sdk_command = match &command {
        ...
        Command::SetEffort { agent, effort } if model.claude_sdk(*agent).is_some() => {
            // The daemon types the effort it forwards, so an effort no offered
            // model knows would fail there; refusing it here keeps the session's
            // facts and in-flight input untouched. Before a model is selected the
            // closed vocabulary is whatever any offered model supports. A session
            // that has offered no catalogue at all has said nothing about efforts,
            // so the request goes to the provider, which is the only side that
            // can answer.
            let facts = facts(model, *agent);
            let offered = if !facts.efforts.is_empty() {
                facts.efforts.contains(effort)
            } else if facts.models.is_empty() {
                true
            } else {
                facts
                    .models
                    .iter()
                    .any(|item| item.efforts.contains(effort))
            };
            if !offered {
                return crate::update::refuse(
                    ...
                    seq,
                    command,
                    "effort is not offered by this session's model",
                );
            }
