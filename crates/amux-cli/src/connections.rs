...
pub fn pairing_identity_lines(pending: &amux::PendingPeer) -> [String; 2] {
    [
...
pub fn pairing_success_line(peer: &amux::PeerEntry, via: amux::PeerVia) -> String {
    let route = match via {
        amux::PeerVia::Direct => "on this network",
        amux::PeerVia::Relay => "through the relay",
        amux::PeerVia::Ssh => "over SSH",
    };
...
fn candidate_via(via: amux::PeerVia) -> &'static str {
    match via {
        amux::PeerVia::Direct => "found · on this network",
        amux::PeerVia::Relay => "seen · through the relay",
        amux::PeerVia::Ssh => "seen · over SSH",
    }
}
...
mod tests {
    ...
    fn onramp_candidate_resolution_accepts_id_and_rejects_ambiguous_names() {
        let first = candidate(5, "phone", amux::PeerVia::Direct);
        let second = candidate(6, "phone", amux::PeerVia::Relay);
        assert_eq!(
    ...
    fn host(id: u128, name: &str, online: bool) -> amux::HostEntry {
        amux::HostEntry {
    ...
    fn peer(id: u128, name: &str, reachability: amux::PeerReachability) -> amux::PeerEntry {
        amux::PeerEntry {
    ...
    fn candidate(id: u128, name: &str, via: amux::PeerVia) -> amux::PairingCandidate {
        let mut host = host(id, name, true);
        host.trust_status = amux::HostTrustStatus::UntrustedButOnline;
        amux::PairingCandidate {
            host,
            via,
            addrs: Vec::new(),
        }
